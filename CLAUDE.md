...
> `.claude/CLAUDE.md`（共通ルール）と同時にコンテキストへ読み込まれる（Claude Code公式仕様: `./CLAUDE.md` `./.claude/CLAUDE.md` は共に project-level の auto-load 対象）。**矛盾する記述がある場合にどちらが優先されるかは公式に保証されていない**ため、本ファイルには共通ルールと矛盾する内容を書かない。
...
> ⚠️ denyパターン一致が ✓ でない本番 alias を設定した場合は、プロジェクト直下に `.prod-aliases`（1行1 alias、`#` コメント可）を作成しそのエイリアス名を追記すること（例: `gf-main`）。`.claude/hooks/pre-operation.js` が読み込みハードブロックに追加する。`settings.json` の deny ルールへの追記は `/upgrade` で上書きされ消えるため非推奨（`.prod-aliases` はプロジェクト固有ファイルのため `/upgrade` の対象外）。
...
- **新規メタデータ（項目・オブジェクト・レコードタイプ・タブ・Apex・フロー）を作成したときは、権限・基本設定（FLS・オブジェクト権限・タブ表示・レイアウト割当・レコードタイプ割当・クラスアクセス等）を必ず確認・設定する**
  - **付与方針の詳細**: `.claude/templates/common/new-metadata-permissions-checklist.md` 参照
