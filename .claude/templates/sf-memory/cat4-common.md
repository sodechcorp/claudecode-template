...
**ファイル粒度の原則（二重収録の禁止）**: 1 機能ID（F-XXX）＝ 1 設計書ファイルを基本原則とする。同一機能IDを複数のファイルに記録してはならない（個別ファイルと複合ファイルの両方、または複数の複合ファイルに同じ機能IDを載せる「二重収録」を禁止）。各機能IDはちょうど 1 つのファイルに帰属する。

- **複合ファイル（例外）**: 単独で文書化する意義が薄い従属コンポーネント（単一の Trigger/Batch から排他的に呼ばれる Handler、組織生成時の自動配備ボイラープレート群等）は呼び出し元・代表コンポーネントに統合してよい。統合時はファイル名に含まれる全機能IDを列挙する（`【F-002〜F-003】name.md`）。種別固有の統合条件は各種別エージェント（apex のハンドラ統合ルール等）に従う。
- **複合ファイルに含めた機能IDは個別ファイルを別途作成しない。かつ 1 つの機能IDが複数の複合ファイルに同時に含まれてはならない**（帰属先は常に 1 ファイル）。
- アップデートモードで既存の個別ファイルと複合ファイルに同一機能IDが重複している場合は、どちらか一方（代表ファイル）に片寄せし、もう一方から当該機能IDを除去する。手動追記・設計判断の根拠は片寄せ先に保持する。
- **相互参照の表記**: 相互参照欄（呼び出し元/先・関連コンポーネント）は `docs/.sf/feature_ids.yml` の F-ID（例: `F-042`）で表記する。旧 CMP- 採番での参照は幻IDになるため禁止。
