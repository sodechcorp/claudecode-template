...
- `release-issue.md` 存在 → 「Phase 5（スモーク確認）から / Phase 6（リリース）から再試行 / 中止 のどれにしますか？」
  - 選択後: `release-issue.md` を Read し、差し戻し理由・現象・ログを把握してから該当フェーズを実施する
...
- `investigation.md` のみ存在 → `investigation.md` フロントマターの `issue_type` を確認し、以下の通り選択肢を提示する:
  ...
  - `issue_type` = `追加要望` / `その他` → 「Phase 1.5（対応記録作成）から / 中止 のどれにしますか？」
