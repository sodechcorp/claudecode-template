...
`/auto-test` Phase B のテスト仕様展開（test-spec.md 生成）で参照する対応表。
`auto-evidence-runner` Step 1 および `backlog-planner` テスト観点列挙でも参照する。
...
| `スクショ目視` | 画面表示の正確な確認（要手動に格下げ推奨） |

---

## skip 可の判断基準

以下の場合は `自動化可否 = 要手動（理由）` とし、skip を許容する:
- 実外部サービスへの実通信が必要
- 本番限定データ・権限セットが前提
- 画面レイアウト/デザインの目視確認（ピクセル単位の差分等）
- Login As が組織設定で無効になっている場合の UI 権限確認
- Salesforce のスケジュール実時刻起動が必要なバッチ
