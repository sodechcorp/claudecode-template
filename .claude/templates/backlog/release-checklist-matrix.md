...
- [ ] 本番リリースの記録: `docs/decisions.md` の当該課題エントリの「リリース予定日 / 担当」欄を実施日に更新する（エントリが無ければ追記する）。`docs/logs/changelog.md` に本番リリース済みである旨がまだ反映されていなければ「日付 / 変更内容 / 関連課題ID」で追記する。xlsx フォルダが設定されている場合は「リリース・ロールバック」シート内「リリース実施記録」欄へ デプロイ日時 / 対象環境 / 結果 を記入する（本番デプロイ結果は Claude Code が代行できないため人間が手動で記入する）
