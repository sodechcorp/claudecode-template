...
- [ ] 本番リリースの記録: `docs/decisions.md` の当該課題エントリの「リリース予定日 / 担当」欄を実施日に更新する（エントリが無ければ追記する）。`docs/logs/changelog.md` に本番リリース済みである旨がまだ反映されていなければ「日付 / 変更内容 / 関連課題ID」で追記する。**この記録は release-preparer Phase 7（デプロイ完了報告を受けて実施）が担当する**。対応記録.xlsx にはリリース実施記録用のシートは存在しない（`patch_template_v8` で廃止済み。本番リリース履歴は decisions.md / changelog.md に一本化）
...
- **リリース前**: dry-run・`with sharing` 確認・埋め込み先（レイアウト/フレックスページ）の確認・上記「原則」の親子セット確認
