...
- **カタログに無い調査手段も自律的に実行**: オプションは最低限の床。バグ非自明では、カタログ外の調査方法も investigator 自身が発想して実行してよい（「採用したオプション」欄に `adhoc-{名前}` で記録）。3 件を超える場合は同欄に超過理由も 1 行記録する（実行を止める上限ではない。詳細: backlog-investigator.md §調査原則）
...
### Phase 1 完了時のスコープ確定による light 格上げ

`--light` を指定せず開始した課題でも、Phase 1（investigator）完了時点で investigation.md の内容から以下を**全て**確認できた場合、`backlog.md`（メインスレッド）は `{light_mode}` を `true` に格上げし、以降のフェーズで `--light` と同じ扱い（Phase 2 スキップ・Phase 3.5 Step A「regression-guard」/ Step C「backlog-validator」スキップ・Step B「Before エビデンス」は UI 影響判定に従い実施）を適用する。上記「典型的自明ケース」（変更種別を typo/ラベル等に限定）とは別軸の判定であり、**変更種別は問わない**（ロジック変更を含んでよい）。investigation.md に書かれた事実のみで判定し、推測・楽観的見込みでは判定しない:
...
**運用**: 条件を満たす場合、Phase 1 完了サマリーの末尾に「スコープが1ファイル・1要素に確定したため、Phase 2・blind レビュー・regression-guard を省略します」と一行で事実として明記する（質問文にしない。[answer-scope-spec.md](../common/answer-scope-spec.md) 準拠）。Phase 1→1.6/1.5 の遷移は既定が軽量承認（異議がなければ次へ）のため、ユーザーは異議があればここで「通常フローで」等と指定すれば全フェーズ実施に戻せる。`{light_mode}` の書き込み手順は `backlog.md` Phase 1 完了時のフロントマター記録を参照。

条件を満たさない場合は `{light_mode}` を `false` のまま通常フローを継続する。判定に迷う場合は格上げしない（取りこぼしより誤判定による安全確認の欠落の方がリスクが大きいため、ここは「迷ったら通常フロー」を適用する）。
