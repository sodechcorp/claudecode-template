...
| 対応内容 | バックアップ情報 (B3-B5) | Phase 4 / implementer | `backup-info` |
...
| テスト・検証記録 | テスト項目（実装前 F/G列） | Phase 3.5 / validator | `test-precheck` |
| テスト・検証記録 | テスト項目（実際の結果 F列・判定 G列） | Phase 5 / tester | `cell --col 6/7` |
...
| リリース・ロールバック | リリース実施記録 | Phase 6 / releaser | `release-record` |
