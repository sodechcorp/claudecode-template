...
**例外: 軽量承認モード**:
低不可逆ゲート（read-only 解析／dry-run で永続副作用なし）であることを常に前提とし、**Phase 4（実装）着手後の遷移のみ** 自明ケース判定 ON かつ軽微修正 4 条件（`quality-gate.md §軽微修正の4条件`）も満たす場合に限り、「異議がなければ進む」を許容する（明示承認テキスト不要）。Phase 4 着手前の遷移（コード・メタデータへの変更が一切発生していない段階）は低不可逆ゲートのみで判定する。**Phase 3.5→4・Phase 5→6（Sandbox デプロイ）・本番デプロイ・お客様サイン・Backlog 投稿は本例外の対象外で常に明示承認**。適用条件・ループ上限の詳細は `backlog.md`【フェーズ進行】内の「軽量承認モード」を参照。
