...
①②のいずれかを満たさない場合は `{light_mode}` を `false` のまま通常フローを継続する。判定に迷う場合は格上げする（2026-09-16変更。①の各条件〔1ファイル・1要素確定・API名/型変更なし・権限/FLS/RecordType等を含まない〕自体は緩めていないため、権限・FLS・共有ルール等の危険種別は①の時点で対象外のまま残る。①②の条件を満たすかどうかの境界判定〔例:「1要素と言えるか微妙」〕でグレーな場合のみ、旧来の「迷ったら通常フロー」から反転し軽量側に倒す。Phase 1→1.6/1.5 の遷移が軽量承認〔異議がなければ次へ〕であるため、誤って格上げされても Phase 1 完了サマリーの一行明記を見てユーザーが「通常フローで」と言えば戻せる）。
