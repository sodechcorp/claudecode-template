...
> - **Phase 1〜3 の■項目（後述）にも本原則が及ぶ**。「丁寧に書く」は説明の分かりやすさ・言葉選びを指すのであって、行数を伸ばしてよいという意味ではない。■1項目は4行以内。結論の1文を含めサマリー全体で15行以内。
...
■ 課題の概要（4行以内）: どんな機能・画面の話か、どういう業務場面で使われているか、なぜ今回起票されたか
  （課題を全く知らない人でも「何の話をしているか」が分かる説明。背景・経緯も含めて丁寧に書く）
■ 前提・背景（4行以内）: 現状の仕様・設定上どう動くことになっているか
■ 実際こうなっている（4行以内）: 観測された現象（根拠: ファイル名・Backlog コメント等）
  （バグ系は「本来こうなるはず → 実際はこうなっている → 原因はここ」の3段で書く）
■ お客さんの要望（4行以内）: どういう仕様にしてほしいと言っているか（出典確認済みの場合のみ断定。未確認は [要確認] を付ける）
```
...
■ 前提（Phase 1 のまとめ・4行以内）: 現状と要望を1行で再掲（このフェーズ単独で文脈が通るように）
■ 行う対応（4行以内）: こういう変更をします
■ 結果こうなる（4行以内）: そうすれば、こういう仕様・挙動になります
■ 根拠（4行以内）: なぜそれで実現できるか（「〇〇が△△するから」——仕様・設定・既存実装に基づく）
```
...
■ 前提（Phase 2 のまとめ・4行以内）: 合意した対応方針を1行で再掲
■ 修正箇所（4行以内）: どのコンポーネント/ファイルをどう直すか（ファイル名のみ可、パス羅列不要）
■ 変わること（4行以内）: 修正によって直接変わる挙動
■ 最終挙動（4行以内）: ユーザー目線で最終的にどう動くか（「〇〇したら△△できるようになります」の形で）
```
...
**例外: 軽量承認モード**:
低不可逆ゲート（read-only 解析／dry-run で永続副作用なし）であることを常に前提とし、**Phase 4（実装）着手後の遷移のみ** 自明ケース判定 ON かつ軽微修正 4 条件（`quality-gate.md §軽微修正の4条件`）も満たす場合に限り、「異議がなければ進む」を許容する（明示承認テキスト不要）。Phase 4 着手前の遷移のうち Phase 1.6→1.5／Phase 1.5→2／Phase 3→3.5（コード・メタデータへの変更が一切発生していない段階）は低不可逆ゲートのみで判定する。**Phase 1→1.6・Phase 2→3・Phase 3.5→4・Phase 5→6（Sandbox デプロイ）・本番デプロイ・お客様サイン・Backlog 投稿は本例外の対象外で常に明示承認**（Phase 1→1.6・Phase 2→3 は次フェーズに永続副作用〔Sandbox への REPRO_ レコード作成・対応記録.xlsx の外部フォルダ生成〕があるため）。適用条件・ループ上限の詳細は `backlog.md`【フェーズ進行】内の「軽量承認モード」を参照。
