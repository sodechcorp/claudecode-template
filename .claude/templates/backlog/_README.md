...
```
【Phase {N} 完了サマリー】
（3〜5 行で成果物の本質・主な発見・次フェーズへの引き渡し要点を要約。§人が読む欄規約に従い日本語ラベル優先で書く。バグ系は「前提→本来→実際→原因」の型で）
...
**運用ルール**: Phase 1.5 で xlsx_folder / evidence_dir が確定した時点で、investigator（または main スレッド）が investigation.md フロントマターを更新する。フロントマター例:
```yaml
---
issue_id: XXX-123
issue_type: bug
