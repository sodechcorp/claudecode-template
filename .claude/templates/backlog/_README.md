...
- backlog-investigator — **呼び出し元（backlog.md）が事前取得**（knowledge-only + 通常モードの両方を Phase 1 開始前に backlog.md が逐次 Task 起動し、結果を渡す。investigator 自身は起動しない。二段ネスト・並列多重発行回避のため。詳細は backlog-investigator.md Step 0a 参照）
...
4. 各オプションの結果を成果物（`investigation.md` / `approach-plan.md` / `validation-report.md` / `test-report.md` 等）に統合
...
- **例外: `default-when-uncertain: skip` を明示できるオプション**: (a) `estimated-cost: 重` かつ実行条件がキーワードヒット型のオプション（subagent 起動・本番 SELECT・全件探索系）、(b) category D（規模・影響範囲限定オプション）のうち `auto-execute-when` が課題内容から確度高く判定できずグレー誤爆リスクが高いもの（例: option-future-extensibility / option-conservative-vs-progressive。小規模課題でも「迷ったら実行」default に倒れて毎回発動していた実害があったため明示・2026-09-15）。いずれも `default-when-uncertain: skip` を明示することで「グレー → スキップ」を選べる。`auto-execute-when` がヒットすれば従来通り実行されるため、明示的な実行シグナルがある時のみ起動する形になる
...
**初期判定条件（investigator の Step 0a-2 で課題本文・コメントのみから判定する。以下を全て満たす）**:
...
**確定条件（初期判定条件を満たした課題のみ、Step 0a-2 の「最小限の調査」内で確認する。いずれかに違反した場合は自明ケース判定を取り消し、通常の Step 0b へ降格する）**:

- 副作用検証: 対象シンボル（メソッド名・コンポーネント名・項目 API 名）を軽量 grep（1 回）し、参照元に追加影響がないこと
...
- API 名変更・型変更
- 複数ファイル横断変更
- 権限・FLS・レイアウト割当・RecordType visibility の変更（アクセス制御に関わるため行数・ファイル数に関わらず対象外）

### Phase 1 完了時の light 格上げ（スコープ確定 ＋ 依頼明確性）

`--light` を指定せず開始した課題でも、Phase 1（investigator）完了時点で investigation.md の内容から**「スコープ」と「依頼明確性」の両方**を満たすと確認できた場合、`backlog.md`（メインスレッド）は `{light_mode}` を `true` に格上げし、以降のフェーズで `--light` と同じ扱い（Phase 2 スキップ・Phase 3.5 Step A「regression-guard」/ Step C「backlog-validator」スキップ・Step B「Before エビデンス」は UI 影響判定に従い実施）を適用する。上記「典型的自明ケース」（変更種別を typo/ラベル等に限定）とは別軸の判定であり、**変更種別は問わない**（ロジック変更を含んでよい）。investigation.md に書かれた事実のみで判定し、推測・楽観的見込みでは判定しない。

**両方を要求する理由**: 「依頼が明確」なだけでは不十分（依頼が明確でも影響範囲が広い変更は regression-guard 等の検証層を省いてよい理由にならない）。「スコープが小さい」だけでも不十分（1 ファイル内でも対応方針が複数考えられる曖昧な依頼なら Phase 2 の検討が要る）。2 つは別の問題を防ぐための別の条件であり、どちらか一方では格上げしない。

**① スコープ（regression-guard 等の検証層省略の条件）**:
- 影響範囲: 変更対象が 1 ファイル・1 要素（1 メソッド/1 項目/1 コンポーネント）に確定している
- 影響範囲テーブル（investigation.md §影響範囲）に区分 S（兄弟入口）の行がない（複数入口に及ぶ場合は対象外）
- API 名変更・型変更を含まない
- 権限・FLS・レイアウト割当・RecordType visibility の変更を含まない

**② 依頼明確性（Phase 2 の対応方針検討省略の条件）**:
- investigation.md「業務要件の不確実点」の Q 番号が 0 件（業務側の判断待ちがない）
- 対応方針が唯一解と判断できる（`option-alternative-approaches` の `auto-skip-when` と同一基準を流用: 種別がバグで原則唯一解、または推奨される対応が既存パターン踏襲以外に技術的・業務的に妥当な代替案がない）

**運用**: ①②両方を満たす場合、Phase 1 完了サマリーの末尾に「スコープが1ファイル・1要素に確定し対応方針も唯一解のため、Phase 2・blind レビュー・regression-guard を省略します」と一行で事実として明記する（質問文にしない。[answer-scope-spec.md](../common/answer-scope-spec.md) 準拠）。Phase 1→1.6/1.5 の遷移は既定が軽量承認（異議がなければ次へ）のため、ユーザーは異議があればここで「通常フローで」等と指定すれば全フェーズ実施に戻せる。`{light_mode}` の書き込み手順は `backlog.md` Phase 1 完了時のフロントマター記録を参照。

①②のいずれかを満たさない場合は `{light_mode}` を `false` のまま通常フローを継続する。判定に迷う場合は格上げしない（取りこぼしより誤判定による安全確認の欠落の方がリスクが大きいため、ここは「迷ったら通常フロー」を適用する）。
...
> - **Phase 1〜3 の■項目（後述）にも本原則が及ぶ**。「丁寧に書く」は説明の分かりやすさ・言葉選びを指すのであって、行数を伸ばしてよいという意味ではない。■1項目は3行以内。結論の1文を含めサマリー全体で10行以内。上限は目標ではなく上限なので、判断に不要な前提説明・当たり前の繰り返しは書かず、できるだけ短く収める。
> - **例外（回答・手順書系の成果物）**: 上記「要点だけ」はプロセスの途中経過（調査・方針検討等の技術詳細）に対する原則であり、**ユーザーがそのまま読んで使う完成品**（問い合わせ回答ドラフト・顧客確認事項ドラフト・本番リリース手順書・管理画面操作手順書等）には適用しない。この種の成果物は要約で済ませず、**ファイル保存に加えてチャットに全文をそのまま提示する**（ファイルを開かせない）。新規コマンド/エージェントで同種の成果物を追加する際もこの例外を踏襲すること。ただし成果物内に**対外送付部分と社内レビュー用部分が併記されている場合**（例: 回答ドラフトの「質問の要約・根拠・[要確認]・補足」）、「全文」は対外送付部分のみを指す。社内レビュー用部分はファイル保存のみに留め、チャットへの再掲は不要（詳細は `backlog.md` Phase 2「問い合わせ」節を参照）。
...
3〜5行で成果物の本質・主な発見・次フェーズへの引き渡し要点を要約する（従来どおり）。ただし上記の例外に該当する回答・手順書系の成果物が含まれる場合は、その部分だけ全文提示に切り替える。
...
■ 課題の概要（3行以内）: どんな機能・画面の話か、どういう業務場面で使われているか、なぜ今回起票されたか
  （課題を全く知らない人でも「何の話をしているか」が分かる説明。背景・経緯も含めて丁寧に書く）
■ 前提・背景（3行以内）: 現状の仕様・設定上どう動くことになっているか
■ 実際こうなっている（3行以内）: 観測された現象（根拠: ファイル名・Backlog コメント等）
  （バグ系は「本来こうなるはず → 実際はこうなっている → 原因はここ」の3段で書く）
■ お客さんの要望（3行以内）: どういう仕様にしてほしいと言っているか（出典確認済みの場合のみ断定。未確認は [要確認] を付ける）
```

**項目の増減禁止**: 上記■4項目以外のセクションを追加してはならない。

**4項目に収まらない必須事項の置き場**: 以下は例外的に■4項目の後ろへ、見出しを付けず1〜2行のプレーンな文として添える（新しいセクションを作らない）。
- 影響範囲テーブルに区分 S の行がある場合の注意書き（`backlog-investigator.md` §フェーズ完了の提示 参照）
- 典型的自明ケース判定時の `--light` 案内
...
■ 前提（Phase 1 のまとめ・3行以内）: 現状と要望を1行で再掲（このフェーズ単独で文脈が通るように）
■ 行う対応（3行以内）: こういう変更をします
■ 結果こうなる（3行以内）: そうすれば、こういう仕様・挙動になります
■ 根拠（3行以内）: なぜそれで実現できるか（「〇〇が△△するから」——仕様・設定・既存実装に基づく）
```
...
- 0 件時の文言: **「特に確認事項はありません」**（validator の validation-report.md 総合判定欄は 0 件時も総合判定の優先順位ルールで確定した3値のいずれか（通常「Phase 4（実装）へ進んでよい」）を記載し、Phase 末尾の確認事項表記は「特に確認事項はありません」に統一）
...
- **一語の曖昧返答全般**: 「ふむ」単体等、文脈から進行肯定が確実に読み取れない語（「ok」は上記「承認とみなすもの」の明確な語に該当するため、ここには含めない）
...
**既定: 軽量承認モード**:
「異議がなければ進む」（明示承認テキスト不要）をフェーズ末の既定とする。**Phase 2→3（対応方針の確定）・Phase 3→3.5（実装方針の確定）・Phase 3.5→4（実装着手）・本番デプロイ・お客様サイン・Backlog 投稿は常に明示承認**（対応方針・実装方針は業務判断そのものであり黙って承認扱いにしない）。**Phase 4（実装）着手後の遷移**（現状 Phase 4→5 のみ該当）は上記に加え、自明ケース判定 ON かつ軽微修正 4 条件（`quality-gate.md §軽微修正の4条件`）も満たす場合のみ軽量承認とし、満たさなければ明示承認に戻す。**Phase 5→6（Sandbox デプロイ）は明示承認を要求しない（2026-09-15変更）**: Sandbox は可逆・低リスクのため、スモーク確認PASSなら承認を待たず自動で Phase 6 へ進む（条件付きPASS＝NoTestRunフォールバック時のみユーザー判断を仰ぐ。詳細は `backlog.md` Phase 5 §次に進む条件）。それ以外の遷移（Phase 0→1／1→1.6／1.6→1.5／1.5→2）は軽量承認が既定。連続自動進行の回数上限は設けない（ユーザーが異議を挟んだ時だけ止める）。詳細は `backlog.md`【フェーズ進行】内の「軽量承認モード」を参照。
...
**追記が不要な内容**:
- Q番号テーブルの内容（investigation.md「業務要件の不確実点」の `- 回答:` 欄、または approach-plan.md「業務要件への回答」欄に記録済みのため重複不要。ただし investigation.md 側は Phase 1 末尾で main thread が `- 回答:` 欄へ書き戻すまでは未記録の状態であることに注意）
