...
> **⛔ 空提出禁止（副作用あり）**: `business_flow[]` を `[]`（空配列）のまま JSON を提出すると、Excel の「業務フロー」シートに **「処理を起動する」という 1 ノードだけの図**が機械生成される。内容が一見埋まっているように見えて実は無意味な図になる。**必ず 5〜8 件書くこと**。書けない場合でも最低 1 件は書く（「（グループ目的）処理を起動する」より「〇〇担当者が〇〇画面から申請する」の方が圧倒的に価値が高い）。
