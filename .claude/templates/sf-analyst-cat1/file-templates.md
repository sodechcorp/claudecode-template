...
### 追加セクション（Phase 2.6 ヒアリングで取得・スキップ時は `**[未ヒアリング]**` マーカー付き空欄）

以下のセクションは Phase 2.6 で取得した値で埋める。スキップした項目はマーカーのみ残置し、後続の解消フロー（Phase 5.5）で再確認する。
...
| 繁忙期 | **[未ヒアリング]** |
| 閑散期 | **[未ヒアリング]** |
| SF 操作禁止期間（年次決算・システムメンテ等） | **[未ヒアリング]** |
...
（直近1年で起きた重大な問題・絶対に繰り返してはいけないこと）

## 対応禁止事項

- **[未ヒアリング]**

（触ってはいけない設定・データ・運用ルール）
```
