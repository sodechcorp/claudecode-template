...
| `title` | ✓ | **業務単位の短い名称（10〜14字目安）**。「見積書承認」「請求書発行」「車両受取・精算」のような業務操作 1件を表す。バッチ時刻・コンポーネント名・データ更新詳細は title に入れず `trigger` / `output` に分離する |
...
> ❌ NG の理由: ①複数バッチ・複数操作を1 title に詰めている ②Apex クラス名をそのまま title にしている ③データ更新詳細を title に書いている（output に分離すべき）
