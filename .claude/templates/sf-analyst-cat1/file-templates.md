...
- **会社・事業概要**: 業種・主要ビジネス・顧客層（推定根拠を明記。Web 由来の情報は `（出典: URL, 取得YYYY-MM-DD）` を付記）
...
- **導入パッケージ一覧**: インストール済み管理パッケージ名・namespace・バージョン（Phase 1-4 の Tooling API クエリ結果。バージョンはここが正本——コード・コメントから推論しない）

### 追加セクション（Phase 2.6 自動抽出・取得不可時は `**[資料未確認]**` / `**[組織未調査]**` マーカー付き空欄）

以下のセクションは Phase 2.6 で資料・組織メタデータ・Web 公開情報から自動抽出した値で埋める。取得できない項目はマーカーのみ残置し、後続の解消フロー（Phase 5.5）で確認を促す。

- `**[資料未確認]**` = 読み込んだ資料に記載が見当たらない
- `**[組織未調査]**` = CLI/SOQL での組織メタデータ取得に失敗した
- Web 由来の値は `値（出典: URL, 取得YYYY-MM-DD）` の形式で記載する（専用マーカーは使わない）
...
> Web から補完した場合の記載例: `約100億円（出典: https://example.co.jp/company, 取得2026-06-01）`
...
| パッケージ名 | Namespace | バージョン |
|---|---|---|
| **[組織未調査]** | — | — |

（Phase 1-4 の `InstalledSubscriberPackage` Tooling API クエリ結果で埋める。取得失敗時は `**[組織未調査]**` のまま残す）
```
...
|---|---|---|---|
| ✅ OK | 見積書承認 | 営業部長が承認ボタン押下時 | 見積書のステータスを「承認済」に更新 |
| ✅ OK | 購入ランク日次更新 | 日次バッチ（毎朝0時） | 顧客の購入ランクを更新 |
| ✅ OK | 工場ボードで作業開始 | 担当者が工場ボード画面を開く | 作業のステータスを「着手」に更新 |
| ✅ OK | LINE 友達追加 | 顧客が友達追加リンクから登録 | 顧客レコードに LINE 連携フラグを立てる |
| ✅ OK | Pardot へリード連携 | 見込客登録時 | マーケティング基盤にリード送信 |
...
> ❌ NG の理由: ①複数バッチ・複数操作を1 title に詰めている ②**技術識別子**（Apex/Aura/LWC/Flow のクラス名・API 名 `*__c`・コンポーネント名）を title に書いている（プロジェクト初参加者には意味不明な識別子） ③データ更新詳細を title に書いている（output に分離すべき）
>
> **✅ OK と ❌ NG の境界**: Pardot / LINE / GMOサイン / Reserve サイト 等の **業務固有名詞・製品名** は許容する。組織内部のメタデータ識別子（`*__c`・Apex クラス名・Aura コンポーネント名）のみ禁止する。「LINE 友達追加」は OK（LINE は業務固有名詞）、「LINE 友達追加URL」は NG（URL を機能名として使っている）。
