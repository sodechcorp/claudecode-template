---
description: "Salesforce 組織情報を分析し、組織概要・オブジェクト構成・マスタデータ・設計書・機能グループ・保守履歴・情報所在マップ・SF標準仕様の8カテゴリを docs/ 配下に生成する。3回の質問で実行カテゴリを選択する。"
---
...
AskUserQuestion ツールを **3回、順番に** 呼び出す（[共通ルール](../CLAUDE.md#askuserquestion-ルール厳守)の「1質問1回答」に従う。1回のコールに複数質問をまとめない）。各回で提示する質問は以下の通り。

**1回目の質問**（`multiSelect: true`）:
...
- **選択肢**:
  ...
  - この質問はスキップ（Cat4以降へ）— Cat1〜Cat3 を実行せず次の質問へ進む

> **「この質問はスキップ（Cat4以降へ）」が選択された場合**: Cat1〜Cat3 の実行をゼロとして扱う（Q1 で他の選択肢と同時に選ばれた場合も Cat1〜Cat3 は実行しない）。
> **次の質問予告**: Q2 で設計書（cat4）・機能グループ定義（cat5）・過去事例参照（cat6）、Q3 で情報所在マップ（cat7）・SF 標準仕様（cat8）を選択できます。

**2回目の質問**（`multiSelect: true`）:
...
- **選択肢**:
  ...
  - この質問はスキップ（Cat7以降へ）— Cat4〜Cat6 を実行せず次の質問へ進む

> **「この質問はスキップ（Cat7以降へ）」が選択された場合**: Cat4〜Cat6 の実行をゼロとして扱う（Q2 で他の選択肢と同時に選ばれた場合も Cat4〜Cat6 は実行しない）。
> **次の質問予告**: Q3 で情報所在マップ（cat7）・SF 標準仕様（cat8）を選択できます。

**3回目の質問**（`multiSelect: true`）:
...
- **選択肢**:
  ...
  - この質問はスキップ（実行しない）— Cat7・Cat8 を実行せず完了する
...
> - **「前提カテゴリを先に実行する」選択時の挙動**: 不足している前提カテゴリ（org-profile.md 不足ならカテゴリ1、catalog/_index.md のみ不足ならカテゴリ2。両方不足していれば両方）を選択カテゴリに自動的に追加し、下記「複数カテゴリ部分選択時の実行順序」の順序（cat1 → cat2 → cat3 → …）に従って追加した前提カテゴリから先に実行してから、元々選択されていたカテゴリを続けて実行する。追加実行した前提カテゴリの完了報告も Step 2 の形式でそのまま出力する。
...
> ※ **単一カテゴリ実行時の changelog.md 軽量追記**（Phase 4 をスキップする **cat1〜cat6 が1件のみ** の場合に実行。マーカー軽量点検と同じ実行条件）:
> - sf-org-analyst Phase 7.5 の changelog 集約（F-4）は Phase 4 実行時にのみ動くため、単一カテゴリ実行時は誰も changelog.md に追記しない設計ギャップがある。この経路ではコマンド自身が代わりに1行追記する。
> - `docs/logs/changelog.md` が存在する場合のみ実行する（存在しない場合はスキップ。初回生成は別フローの対応範囲）。
> - Edit ツールで先頭コメント行の直後に以下フォーマットで1行挿入する（Write による全文上書き禁止。sf-org-analyst Phase 7.5 と同一フォーマット）:
>   ```
>   - {YYYY-MM-DD}: /sf-memory {実行カテゴリ（cat番号）} — {エージェントの完了サマリより生成/更新ファイル件数}
>   ```
>   例: `- 2026-08-19: /sf-memory cat2 — オブジェクト定義書 18件`
