...
**「Other」・自由入力時の処理（AskUserQuestion を再発行しない）**: 第1質問の回答に「Other」が含まれる、またはユーザーが自由入力テキストを返した場合、以下のルールで解釈してそのまま次の cat6 確認質問に進む。テキスト確認の AskUserQuestion は絶対に発行しない。
- 「すべて」「全部」「全カテゴリ」「全選択」「all」→ cat1〜4 全選択として扱う
- カテゴリ名・番号（「cat1」「1」「組織概要」「設計」等）が読み取れる → 該当カテゴリを選択として扱う
- 判断不能（空欄・無関係なテキスト）→ cat1〜4 全選択として扱う（フォールバック）
