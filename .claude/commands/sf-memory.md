...
### Step 0-1: カテゴリ選択（第1質問）

AskUserQuestion を **1回目** として呼び出す（**multiSelect: true**）。
...
回答を受け取ったら、**テキスト応答を返さず直ちに Step 0-2 に進む**。「Other」や自由入力テキストが返った場合も同様（下記ルールで解釈して進む）:
- 「すべて」「全部」「全カテゴリ」「all」→ cat1〜4 全選択
- カテゴリ名・番号（「cat1」「1」「組織概要」「設計」等）→ 該当カテゴリを選択
- 判断不能 → cat1〜4 全選択（フォールバック）

### Step 0-2: cat6 確認（第2質問）

**Step 0-1 の回答内容に関わらず、必ず** AskUserQuestion を **2回目** として呼び出す。
