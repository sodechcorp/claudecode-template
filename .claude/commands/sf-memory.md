...
- **header**: 「カテゴリ選択2」
- **選択肢**:
  - 設計書生成（cat4）— docs/design/ 配下の Apex・Flow・LWC 等の設計書を生成
  - 機能グループ定義（cat5）— docs/.sf/feature_groups.yml を生成（Cat4 完了後 or 単独再生成）
...
> - cat1〜6 が全て選択の場合: **追加質問**（↑ 上記2種類のオブジェクト/コンポーネント指定のみ）をスキップし、全オブジェクト・全機能でそのまま進む（**資料確認質問はスキップしない** — 全選択パターンで常に実施する）
