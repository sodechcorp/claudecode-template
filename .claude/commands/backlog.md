...
>   2. 「特に確認したい点」を **0〜3 個**テキストで挙げる。確認事項がなければ「特に確認事項はありません」と明記し、無理やり挙げない。実装詳細・テスト段取り・派生事項・影響範囲調査結果・スコープ自明事項は確認質問に含めず本文に記載する（責務境界の詳細は各エージェント定義を参照）。**このルールは Phase 1〜6 すべてのフェーズ末に適用する**（investigator / planner / validator / implementer / tester / releaser の全エージェント共通）。**確認に含めるもの**: 業務判断・データ運用判断・スコープ判断（例: 過去データの扱い・業務ルール解釈・受入条件・適用範囲）。**確認に含めないもの**: 実装側で自明判断できる事項（例: テストクラス追加要否・命名・既存パターン踏襲・カバレッジ要件・grep して確認するだけの調査）と採用案の最終確定そのもの（Step 4 の「Phase N に進んでよろしいですか？」で兼ねる）
