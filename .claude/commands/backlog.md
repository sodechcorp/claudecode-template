...
| Phase 5: スモーク確認 | `backlog-tester`（内部: `sf-context-loader`） | スモーク結果（PASS で停止。`/test` は別セッションで実行を提案） |
...
> **目的**: 実装後にデプロイが通っているか・Apex テストが壊れていないか・明らかな実装ミスがないかを短時間で確認する。証跡採取・エビデンス xlsx 生成は行わない。スモーク PASS 後は別セッションで `/test {issueID}` を起動して網羅的テストを実施する。
...
スモーク確認の結果を報告する:
- **PASS** → Phase 5 完了。**このセッションはここで一旦終了**。別セッションで `/test {issueID}` を 1 から起動して網羅的テスト・証跡採取・エビデンス Excel 生成を実施してください。`/test` PASS 後は `/backlog {issueID}` を再実行し「途中フェーズから再開」で Phase 6（Sandbox リリース・完了）に進んでください。
- **FAIL** → Phase 4 に差し戻す（`/test` は実行しない。明らかな壊れを修正してから再度スモーク確認）

> **フロー**: Phase 5 PASS でこのセッションのフローは終了。`/test` は別セッション。`/test` PASS 後に `/backlog {issueID}` を再実行し「途中フェーズから再開」で Phase 6 へ進む。
>
