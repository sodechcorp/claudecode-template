---
description: "Backlog課題の調査・対応・記録を一気通貫で実施する。専門エージェントを順に起動し、各フェーズ完了後にユーザ確認を取りながら進める。/backlog [課題ID] で個別課題対応 / /backlog summary で工数サマリー。"
---

# /backlog [課題ID] または [summary]

**モード判定**: 引数が `summary`（大文字・小文字問わず）なら [summary] モードを実行する。`--light` フラグが付いている場合（例: `/backlog GF-123 --light`）は [課題ID] モードを軽微修正ショートカットで実行する（Phase 2 / Phase 3.5 をスキップ）。それ以外は [課題ID] モード（通常）を実行する。
...
| Phase 1.6: Sandbox 仮説検証 | `backlog-investigator`（Phase 1.6 モード） | `hypothesis-verification.md`（バグ系のみ） |
...
`docs/logs/effort-log.md` を読み込み、以下の形式でサマリーを出力する。ファイルが存在しない場合は「工数ログがまだ存在しません（docs/logs/effort-log.md が未作成です）。/backlog [課題ID] で最初の課題対応を実施すると Phase 2 完了時に自動生成されます。」と出力して終了する。
...
- 見込み合計
- 実績合計（記録ある分のみ）
- 実績/見込み比（Σ実績 / Σ見込み × 100%）（記録ある行のみ）
```

古いスキーマ（種別・対応者種別 列なし）の行は `種別=不明`、`対応者種別=不明` として読み込む。

範囲表記（例: `4〜6h`）が混在する場合は **中点**（`(min+max)/2`）で数値化して集計する。
...
工数見込みを `docs/logs/effort-log.md` に記録する。以下の手順で実施する:

**Step 1: 担当者名の取得**
`mcp__backlog__get_myself` を呼び出し、レスポンスの `name` を担当者名とする。失敗した場合はルート `CLAUDE.md` の `effort_log_author` 設定値を参照する。両方取得できない場合は `不明` を入れる。

**Step 2: 重複チェック**
`docs/logs/effort-log.md` が存在する場合、`{issueID}` が既に含まれているか確認する:
- **既存行あり（再実行ケース）**: Edit で当該行の「見込み」列を最新の採用案の値に更新する。追記はしない
- **既存行なし**: 以下 Step 3 の形式で最終行の後へ追記する

**Step 2.5: スキーマ確認・自動マイグレーション（v2 対応）**

`docs/logs/effort-log.md` が存在し、かつ以下の **全条件** を満たす場合のみマイグレーションを実行する（プロジェクト固有のカスタムスキーマには触れない）:
- ヘッダー行に `種別` 列が含まれる（v1 標準スキーマの識別子）
- ヘッダー行に `対応者種別` 列が含まれる（v1 標準スキーマの識別子）
- ヘッダー行に `採用アンカー` 列が含まれない（v2 未適用を確認）

上記全条件を満たす場合（= v1 標準スキーマ確認済み）:
- Edit でヘッダー行・区切り行を 10 列（v2 スキーマ）に拡張する
- 既存データ行の末尾に `| - | - |` を追記して 10 列に揃える
- 変更後のヘッダー行:
  ```
  | 日付 | 課題ID | 種別 | 対応内容 | 見込み | 実績 | 対応者種別 | 担当 | 採用アンカー | 信頼度 |
  ```

上記条件を満たさない場合（カスタムスキーマ / v2 適用済み / ファイルなし）: Step 3 に進む。カスタムスキーマの effort-log.md には追記しない（`採用アンカー / 信頼度` の記録はスキップ）。

**Step 3: 追記形式（v2 スキーマ・10 列）**
```
| {YYYY-MM-DD} | {issueID} | {issue_type} | {対応内容を1行で} | {Nh} | - | ClaudeCode | {担当者名} | {採用アンカーID（カンマ区切り）} | {高/中/低} |
```

- 「種別」は `{issue_type}` の値（バグ / 追加要望 / その他）
- 「見込み」は sf-effort-estimator が返した値（単一値 `Nh`）
- 「実績」はこの時点では `-` を入れる（Phase 6 でユーザーから聞き取って後埋め）
- 「対応者種別」は `/backlog` コマンド実行の場合は `ClaudeCode` を入れる
- 「採用アンカー」は sf-effort-estimator が返した採用アンカーの issueID をカンマ区切りで入れる。なければ `-`
- 「信頼度」は sf-effort-estimator が返した信頼度（高 / 中 / 低）を入れる

ファイルが存在しない場合はヘッダー行・区切り行・フォーマットコメントを先に作成してから追記する:

```
<!-- 見込み・実績の許容フォーマット: Nh（小数可、例: 1.5h）。範囲表記（例: 3〜4h）は非推奨（集計時に中点変換される） -->
<!-- v2 schema: 採用アンカー・信頼度列を追加。旧 8 列の行は新列を空（-）とみなして集計に影響しない -->
| 日付 | 課題ID | 種別 | 対応内容 | 見込み | 実績 | 対応者種別 | 担当 | 採用アンカー | 信頼度 |
|---|---|---|---|---|---|---|---|---|---|
```
...
**effort-log.md の見込み突合（Phase 3 完了時）**

`implementation-plan.md` が確定した後、Phase 2 で記録した見込みと齟齬が生じていないか確認する:

- `docs/logs/{issueID}/approach-plan.md` の採用案「見込み工数」と `docs/logs/effort-log.md` の当該行「見込み」を照合する
- 値が異なる場合（Phase 3 で方針変更が発生したケース）: Edit で effort-log.md の当該行「見込み」「採用アンカー」「信頼度」の 3 列を implementation-plan.md の最終確定値（sf-effort-estimator 再委譲が必要な場合は再委譲して取得）に更新する
- 一致している場合: 変更不要
...
エージェントが `validation-report.md` を保存したら内容をユーザに提示する。Phase 3 への戻りが提案された場合は Phase 3 に戻って実装方針を修正してから Phase 3.5 を再実施する。**Phase 3 戻りは最大 2 回まで・セッション跨ぎを含めて通算カウント**（カウントは discussion-log.md の改版履歴から復元する。詳細は `test-fail-routing.md` L18 を参照）。3 回目以降の戻り提案が出た場合は自動進行を停止し、「実装方針の見直しが繰り返されています。一度オフラインで方針再検討の打ち合わせが必要かもしれません。このまま Phase 3 に戻りますか？（続行 / 中止）」とテキストで確認する。「続行」ならば Phase 3 に戻る。「中止」ならばコマンドを終了する。
...
>
> **Phase 3.5 典型例**: 「新規発見した影響箇所への対処方針」「エビデンスが取れていない項目の扱い」
...
1. **decisions.md** — Phase 5 まで到達済みか確認する。`option-knowledge-extraction`（Phase 5 always-run）が実行済みなら `docs/decisions.md` に既にエントリがあるため**重複追記しない**。Phase 5 未到達の場合のみ `backlog-releaser.md` Step 3（L165-177）のフォーマットで `docs/logs/{issueID}/approach-plan.md` / `implementation-plan.md` を読んで追記する（前工程ファイルなしフォールバック内蔵）。

2. **pitfalls.md** — `backlog-releaser.md` Step 3.6（L217-258）の手順で実行する:
...
3. **cases/{issueID}.md** — `backlog-releaser.md` Step 3.8（L287-332）の手順で実行する:
   - `docs/knowledge/cases/{issueID}.md` が既存ならスキップ
   - `docs/logs/{issueID}/` 内の現存ファイルから生成（前工程ファイルなしフォールバック内蔵）

4. **case-index.md** — `backlog-releaser.md` Step 4.5（L371-401）の手順で実行する:
   - パスは **`docs/knowledge/case-index.md`**（`cases/` 配下ではない）
   - 実績工数が未確定なら Phase 2 見積り値を `(見込み)` 付きで記入、または `未確定` とする
   - 既存行ありならスキップ（dup 防止）

**実行しないもの**: deploy 系（Step 1・2a/2b/2c）・お客様確認サイン取得（Step 3.7）・xlsx リリース記録（Step 3.5）・完了報告（Step 4）。
