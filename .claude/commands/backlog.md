...
| Phase 1.7: Sandbox 再現確認 | （本コマンド直接実行） | 再現エビデンス（バグのみ） |
...
`{issue_type}` は Phase 1 完了時に確定済み（絶対ルール参照）。途中再開時など未確定の場合は `docs/logs/{issueID}/investigation.md` の「種別」欄から読み取る。`{issue_type}` が `バグ` 以外（追加要望 / その他）の場合は本 Phase をスキップして Phase 2 へ。
...
- **バグの場合**: 再現手順を実機で実施し、画面スクリーンショット・コンソールログ・対象レコード値を取得し `{evidence_dir}/before/` 配下に保存（ファイル名は連番付き。Phase 1.7 の Sandbox 再現エビデンスは `before/repro/` に保存済みなので重複しない）
