...
**各エージェントの内部構造**: 全エージェント（`backlog-repro-runner` を除く）は Step 0b でフェーズ用 `_index-phase{N}.md` を読んでオプション判定を行う（[à la carte 仕組み](../templates/backlog/_README.md)）。`backlog-repro-runner` は Phase 1.6（バグ系のみ）専用で Step 0b を持たず、à la carte 判定の対象外。`backlog-implementer` / `backlog-tester` / `backlog-releaser` はさらに Step 0a で `sf-context-loader` を呼び出す。`backlog-investigator` / `backlog-planner` も Step 0a で `sf-context-loader` を呼び出すが、こちらは knowledge-only モード（ナレッジ層のみ先読み・設計層のフルロードはしない。docs/ の全件読みは investigator の Step C 等で別途実施）。Phase 1.5 は本コマンドが直接実行するためエージェントを起動せず、`_index-phase1-5.md` は存在しない（不要）。

> **`backlog-validator` はサブエージェントを起動しない（leaf agent）**: サブエージェントがさらに別のサブエージェントを起動する二段ネストは不安定なため、Phase 3.5 で必要な `regression-guard`・`ui-evidence-runner`（Before-only）は本コマンド（メインスレッド）が validator の起動前に直接 Task 起動し、結果を validator の起動パラメータとして渡す。詳細は Phase 3.5 セクション参照。
...
>   - **適用条件（AND）**: ① 課題が `_README.md §典型的自明ケース定義` に該当（自明ケース判定 ON） ② `quality-gate.md §軽微修正の3条件` を全て満たす ③ 次フェーズが read-only 解析または dry-run（永続副作用なし）の低不可逆ゲート
...
エージェントが `answer-draft.md` を保存したら、**チャットにドラフト全文をそのまま提示**する。提示の冒頭に必ず次を明記する:
...
> **📋 本番リリース後 TODO**（`{xlsx_folder}` が設定されている場合のみ）: 本フローは Sandbox リリースまで。**本番リリースは人間が手動で実施する**ため、本番デプロイ後に `{xlsx_folder}` の「リリース・ロールバック」シート内「リリース実施記録」欄へ **デプロイ日時 / 対象環境 / 結果** を忘れず記入すること（本番デプロイ結果は Claude Code が代行できない）。
