...
>   2. 「特に確認したい点」を **0〜3 個**テキストで挙げる。確認事項がなければ「特に確認事項はありません」と明記し、無理やり挙げない。実装詳細・テスト段取り・派生事項・スコープ自明事項は確認質問に含めず本文に記載する（責務境界の詳細は各エージェント定義を参照）。**確認に含めるもの**: 業務判断・データ運用判断・スコープ判断（例: 過去データの扱い・業務ルール解釈・受入条件・適用範囲）。**確認に含めないもの**: 実装側で自明判断できる事項（例: テストクラス追加要否・命名・既存パターン踏襲・カバレッジ要件）と採用案の最終確定そのもの（Step 4 の「Phase N に進んでよろしいですか？」で兼ねる）
...
>
> **特に確認したい点に含めてよい例**: 「業務要件 Q1 への仮説が正しいか」「データ X の例外時挙動を業務側に確認したい」
...
>
> **特に確認したい点の例（良い）**: 「過去 X 件のデータで項目 Y が null のレコードを許容するか / 一括補完するか」「未確定の業務ルール Q1 の回答が方針の前提と合っているか」「スコープに含めるべきか別 Backlog で起票するか」
> **確認してはいけない例（悪い）**: 「テストクラスを追加するか」（→ 実装側で判断する）「採用案を確定してください」（→ Step 4 の「Phase N に進んでよろしいですか？」で兼ねる）「命名はこれで良いか」（→ 実装側で判断する）
...
エージェントが `validation-report.md` を保存したら内容をユーザに提示する。Phase 3 への戻りが提案された場合は Phase 3 に戻って実装方針を修正してから Phase 3.5 を再実施する。Phase 3 への戻りは最大 2 回まで（セッション内のみカウント。新セッションでは自動的に 0 から開始）。3 回目の戻り提案が出た場合は処理を中断し、「実装方針の見直しが繰り返されています。ユーザ判断が必要です。このまま Phase 3 に戻りますか？（続行 / 中止）」とテキストで確認する。「続行」ならば Phase 3 に戻る。「中止」ならばコマンドを終了する。
