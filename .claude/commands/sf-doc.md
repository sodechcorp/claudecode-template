...
**中断時のリカバリ（最大1回のみ）**: sf-doc-overview-writer の応答が「⚠️ プロジェクト概要書 生成中断」で始まる場合、そのまま完了報告として出力せず以下を行う。
...
1. 応答内の【理由】をそのままユーザーに提示したうえで、AskUserQuestion で確認する:
   ...
   - options:
     ...
     - label: "このまま終了する"、description: "概要書は生成せず終了する"
2. 「値を修正して再実行する」の場合: チャットで「どの値をどう修正しますか？」と確認し、回答内容に応じて `author` / `version_increment` / `source_file` のうち該当するものを補正した上で、sf-doc-overview-writer を **1回だけ** 再度呼び出す。
3. 再実行後の応答が再び「⚠️ プロジェクト概要書 生成中断」で始まる場合、または手順1で「このまま終了する」を選んだ場合は、その中断報告をそのまま完了報告として出力して終了する（**再々実行はしない**）。
4. 再実行後の応答が「✅ プロジェクト概要書 生成完了」で始まる場合は、その応答を完了報告として出力する。
