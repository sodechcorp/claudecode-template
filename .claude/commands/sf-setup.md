...
`skip` が選択された場合: 「組織認証をスキップしました。後で /sf-setup を再実行してください」と案内し、Step 2〜4 を実行せずここで終了する。
...
**alias 名の予約語チェック（必須）**: 確定した `alias` が厳密に `prod` または `dev` と一致する場合はそのまま使わない。`scripts/setup-sf-project.sh` はこの2語を単体実行時の組織種別ショートカット（予約語）として扱い、第2引数（Sandbox指定）を無視してログインURLを固定するため、Step 1 で選んだ組織種別と接続先が食い違う（`prod`→本番URL固定、`dev`→Sandbox URL固定）。該当する場合はチャットで別名を入力してもらい、`alias` を更新してから Step 3 に進む。
...
> **注意**: 「今どの組織に接続中か」というライブ状態をルート `CLAUDE.md` に固定値で記録しない。接続先は `sf org display` のライブ確認で判定するため、スナップショットを焼き込むと切り替え後に誤判定を起こす。alias と環境（開発/ステージング/本番）の対応表そのものは `CLAUDE.md` の「Salesforce組織情報」セクションに記入してよい（Step 4-2 の案内対象。`setup.sh` の「Sandbox alias 等を記入する」も同じ表を指す）。
