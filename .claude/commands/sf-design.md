---
description: "Salesforce 設計書（詳細設計・プログラム設計・機能一覧）を生成する。詳細設計/プログラム設計/機能一覧の3種別をマルチセレクトで選択して実行。引数に「レビュー」を含めるとプログラム設計JSONをreviewerでクロスチェックする（任意・既定OFF）。"
argument-hint: "[レビュー]"
---
...
```bash
python -c "import yaml, pathlib; p = pathlib.Path(r'{project_dir}/docs/.sf/sf_config.yml'); d = ((yaml.safe_load(p.read_text(encoding='utf-8')) or {}) if p.exists() else {}); (print('━━ 前回の設定値（sf_config.yml）━━'), (print('  作成者名       :', d['author']) if d.get('author') else None), (print('  出力先フォルダ  :', d['output_dir']) if d.get('output_dir') else None), (print('  プロジェクト名  :', d['project_name']) if d.get('project_name') else None), print('↑ 各項目で「前回値を使用」を選ぶと上記の値が引き継がれます')) if d else None"
```
...
```bash
python -c "import pathlib; selected = {selected_types}; output = pathlib.Path(r'{output_dir}'); c1 = list((output / '02_詳細設計').glob('*.xlsx')) if ('詳細設計' in selected and (output / '02_詳細設計').exists()) else []; c2 = list((output / '03_プログラム設計').glob('*.xlsx')) if ('プログラム設計' in selected and (output / '03_プログラム設計').exists()) else []; c3 = [output / '01_基本設計' / '機能一覧.xlsx'] if ('機能一覧' in selected and (output / '01_基本設計' / '機能一覧.xlsx').exists()) else []; print('HAS_EXISTING:', len(c1 + c2 + c3) > 0)"
```
...
以下の内容で `{output_dir}/.tmp/detect_project_name.py` を Write する:
```python
...
```
```bash
python {output_dir}/.tmp/detect_project_name.py
```
...
以下の内容で `{output_dir}/.tmp/save_sf_config.py` を Write する:
```python
import pathlib, sys
...
```
```bash
python {output_dir}/.tmp/save_sf_config.py
```
...
```bash
python -c "import pathlib, sys; p = pathlib.Path(r'{project_dir}') / 'docs' / '.sf' / 'feature_ids.yml'; sys.exit(print('ERROR: docs/.sf/feature_ids.yml が見つかりません。先に /sf-memory を実行してください。') or 1) if not p.exists() else print('OK')"
```

`feature_groups.yml` の存在確認（無ければ `/sf-memory` の先行実行を案内してエラー終了）:
```bash
python -c "import pathlib, sys; p = pathlib.Path(r'{project_dir}') / 'docs' / '.sf' / 'feature_groups.yml'; sys.exit(print('ERROR: docs/.sf/feature_groups.yml が見つかりません。\n       先に /sf-memory を実行して sf-analyst-cat5 に生成させてください。\n       （feature_groups.yml は業務機能グループ定義の正本で、手動整理された内容を含むため自動上書きしません）') or 1) if not p.exists() else print('OK')"
```

FG 一覧をマークダウン表 + in-band signal で出力（**次の assistant メッセージにそのまま貼る**）:

以下の内容で `{output_dir}/.tmp/print_fg_table.py` を Write する:
```python
import yaml
...
```
```bash
python {output_dir}/.tmp/print_fg_table.py
```
...
以下の内容で `{output_dir}/.tmp/resolve_group_ids.py` を Write する:
```python
...
```
```bash
python {output_dir}/.tmp/resolve_group_ids.py
```
...
```bash
python -c "import json, pathlib; p = pathlib.Path(r'{project_dir}/.sf/config.json'); print('alias:' + ((json.loads(p.read_text(encoding='utf-8')).get('target-org') or '') if p.exists() else ''))"
```
...
以下の内容で `{output_dir}/.tmp/check_org_auth.py` を Write する:
```python
import sys, json
try:
...
```
```bash
sf org display --target-org "{sf_alias}" --json 2>/dev/null | python {output_dir}/.tmp/check_org_auth.py
```
...
- options:
  ...
  - label: "対象を絞る（F-XXX またはコンポーネント名）"、description: "機能ID（F-XXX）またはコンポーネント名（API 名）をカンマ区切りで入力（次の質問で聞く）"

「全機能」の場合は `target_ids = []`（全件）。「対象を絞る」または Other が選ばれた場合は F-XXX またはコンポーネント名（API 名）をカンマ区切りで入力してもらう（例: `F-001, AccountTrigger, MyLwc`）。

入力後、以下のリゾルバスクリプトで `target_ids` を確定する:

以下の内容で `{output_dir}/.tmp/resolve_target_ids.py` を Write する:
```python
import yaml, sys, pathlib
inputs = [x.strip() for x in '{入力値}'.split(',') if x.strip()]
fids_path = pathlib.Path(r'{project_dir}') / 'docs' / '.sf' / 'feature_ids.yml'
api_to_fid = {}
if fids_path.exists():
    data = yaml.safe_load(fids_path.read_text(encoding='utf-8')) or {}
...
```
```bash
python {output_dir}/.tmp/resolve_target_ids.py
```

`feature_id:` 以降を集めて `target_ids` に設定する。

`error:` がある場合は AskUserQuestion で以下の3択を提示する:
- question: "機能ID解決エラーが発生しました。どうしますか？"
- header: "エラー対応"
- multiSelect: false
- options:
  - label: "入力を修正して再試行"、description: "Step 0-5 の入力選択に戻る"
  - label: "エラー分を除外して続行"、description: "resolved に含まれる F-XXX のみを target_ids に設定して次へ進む"
  - label: "中止"、description: "エラー内容をユーザーに伝えて終了する"

> グループID（FG-XXX）は機能ID解決では受け付けない。グループ単位の絞り込みは「詳細+プログラム」選択で Step 0-3 を使うこと。
...
- 詳細設計選択時にコンポーネント名（API名・F-XXX）で対象を指定した場合は Step 0-3 のグループ解決スクリプトにより FG-XXX に変換し、確定済みの `target_group_ids` を step1 に渡す
- プログラム設計のみ選択時にコンポーネント名（API名）または F-XXX で対象を指定した場合は Step 0-5 のリゾルバスクリプトにより F-XXX に統一し、確定済みの `target_ids` を step2 に渡す
