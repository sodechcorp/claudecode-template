...
| **可否判断が複数の前提の連鎖で成り立つ場合（「〇〇すれば××できる」等）** | 結論を構成する前提を1個ずつ分解し、**末端の前提まで個別に検証してから結論を書く**。手前の前提（例: 画面が表示ラベルで並ぶ）に出典があっても、その先の前提（例: そのラベル自体を変更できるか・変更が対象画面に反映されるか）が未検証なら、全体を「確認済み」「成立」と報告しない。実機・組織で検証可能な前提は理論値・部分確認で代替せず実際に操作して確認する。前提の一部が未検証なら「〇〇までは確認、△△（具体的な前提）は未検証」と分けて報告し `[要確認]` を付ける |
