...
| **権限系の「できない／直った」の完了判定** | **対象ユーザーの全アサイン（プロファイル＋所属する権限セット＋権限セットグループ）の和集合を確認する**。加えて、異なる権限経路のユーザー（例: プロファイルのみで付与 vs 権限セット経由で付与）を最低1人ずつ用意し、**実ユーザーでの Login As による UI 操作確認**で双方の症状の再現・解消を確認してから完了と判定する。ObjectPermissions/FieldPermissions 等の権限オブジェクトを読解しただけでは完了と判定しない。単一ユーザー・単一経路の確認のみで「直った」「できない」と断定しない |
