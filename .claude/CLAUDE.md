...
**メインスレッド直接応答ガード（最優先チェック）**: この組織固有の事実（業務・データ・処理・運用・活用状況・経緯・人物）を含みうる質問は、**SF キーワードの有無にかかわらず記憶で答えず `docs/` を参照する**（業務理解 0 モード参照）。判断に迷う場合は「固有事項を含みうる」側に倒す。**一見すると一般知識に見えても、答えが組織の事業内容・取扱商品・サービス・顧客・税区分・売上規模に依存するなら「固有事項を含みうる」と判定し、`docs/_README.md` → 該当ファイルを読んでから答える**（例: 「消費税が上がっても食料品は無関係？」→ 取扱商品/サービスが何かは `org-profile.md` を読むまで断定不可）。うち SF キーワード（`__c` / `F-` / `CMP-` / `UC-` / `FR-` / オブジェクト名 / 「自動化」「権限」「ガバナ」等）を含む業務系質問は **assistant に委譲する**（次の例外条件を満たさない限りメインスレッドで完結させない）のが原則。**例外（直答許容）**: 以下の全てを満たす場合のみメインスレッドが直接 Read して答えてよい — ① 1 ファイルの Grep + 部分 Read で答えが確定する ② 複数ファイル横断・設計意図の解釈を要さない ③ ユーザーの依頼が「調査して」「詳しく」「原因は」「経緯は」等の**調査・分析を求めるものではなく**、単純な事実確認（「この項目の用途は？」「この略称は何の意味？」等）。**調査・分析系の依頼は例外を使わず必ず assistant に委譲する**（委譲後の Step 0c ルール一式が調査の精度を担保するため）。**メインスレッドが直答する場合（例外時含む）でも §実装裏付け・出典確認 §調査尽くしゲート（下記）を適用する。docs を軽く読んだだけで推論で締めない**。
