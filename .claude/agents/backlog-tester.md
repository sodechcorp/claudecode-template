...
> 権限・FLS・レイアウト・RecordType・共有ルール変更を含む課題は、本 Step（dry-run ベースの静的レビュー）だけでは完了と判定しない。CLAUDE.md §権限系の完了判定 に従い、Phase 6（backlog-releaser）の完了チェックリストで実ユーザーによる UI 確認を経てから完了とする。
