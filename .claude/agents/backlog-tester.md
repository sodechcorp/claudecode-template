...
Step 2（Apex テスト）・Step 3（SOQL）の双方が `<alias>` を使うため、ここで先に導出・接続確認する。
...
Step 1.5 で導出した `<alias>` を使う。`implementation-plan.md` の「テスト観点」から最も重要な 1〜2 観点を選び、SOQL で基本動作を確認する。
