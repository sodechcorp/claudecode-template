...
Playwright MCP が利用可能な場合（判定: `mcp__playwright__browser_snapshot` を試行呼び出しし、ツールが存在しない・エラー応答の場合は「利用不可」とみなす）、手動確認と並行して自動テストも実施する:
...
- 修正後のレコード値・ログ
- xlsx エビデンスシートの「実装後エビデンス」欄に貼付（`{xlsx_folder}` が設定されている場合のみ。未設定の場合はスキップ）
...
`{xlsx_folder}` が空文字・未設定、または変数名のまま展開されていない場合（`{xlsx_folder}` という形式のまま）、あるいは `{issueID}` が変数名のまま展開されていない場合（`{issueID}` という形式のまま）はこのステップをスキップする。
