...
### Phase 2a: グループ一覧データの取得

下記 3 つの python を順に実行する（存在確認 2 つ → FG 一覧データ取得 1 つ）。
...
FG 一覧を AskUserQuestion 埋め込み用に出力:
```bash
python -c "
import yaml
...
for g in groups:
    fg_id = g['group_id']
    name = g['name_ja']
    cnt = len(g.get('feature_ids', []))
    print(f'  {fg_id}  {name}（{cnt}コンポ）')
print(f'TOTAL={len(groups)}')
"
```

stdout の `TOTAL=` 行から `{n}` を取り出す。それ以外の行（各 FG の 1 行テキスト）を Phase 2b の `question` フィールドに直接埋め込む。

---

### Phase 2b: 対象グループの選択

Phase 2a の FG 一覧テキストを **`question` フィールドに直接埋め込んで** AskUserQuestion を呼ぶ。
ユーザーは質問画面に FG 一覧が表示された状態で選択できる（FG-ID を事前に知っている必要がない）。
...
AskUserQuestion:
  question: |
    ...
    【機能グループ一覧】
    {Phase 2a で取得した FG 一覧テキスト（FG-001〜FG-CMN の各行をそのまま貼る）}
  ...
  options:
    - label: "全グループ（{n}グループ）"
      description: "feature_groups.yml の全グループを対象"
    - label: "グループIDを指定"
      description: "上の一覧から FG-XXX をカンマ区切りで入力（次の質問で聞く）"
    - label: "コンポーネントを指定"
      description: "Apex名・LWC名・F-XXX等で指定（次の質問で聞く）"
```
