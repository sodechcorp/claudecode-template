...
> 詳細スペック: [実装裏付け・出典確認](../CLAUDE.md#実装裏付け出典確認全エージェント共通常に適用) / [実装裏付け](../CLAUDE.md#実装裏付け出典確認全エージェント共通常に適用)。SF 関連・業務理解系タスクでは Step 0c で全文を Read する。
...
- **関連コンテキストが返った場合**: 回答に以下を必ず反映する: (1) 取得内容を回答に引用する際は出典を**ファイルパス単位**で明示する（例: `docs/overview/org-profile.md`）。loader 要約は行番号を保持しないためファイルパス単位で足りる。挙動・仕様を**断定する箇所**は該当ファイルを直接 Read して `ファイル名:行番号` を付す（→ `.claude/CLAUDE.md#実装裏付け出典確認全エージェント共通常に適用`） (2) 取得情報と既知情報が矛盾する場合は両方を提示してその旨を明示する (3) 推測で補わず、docs に記載がない事項は「資料に記載なし」と明示する
