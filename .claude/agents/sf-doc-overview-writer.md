...
>
> 本書の中身は **docs/ 配下の精度に完全依存** する。docs が薄いと骨組みだけになる。図エリアはプレースホルダーのみ（手動貼り付け想定）。最新化は /sf-memory カテゴリ1・2 で行う（sf-doc コマンド側で確認済み）。

以下の内容で `{output_dir}/.tmp/check-docs-exist-overview.py` を Write する:
```python
import pathlib
...
```
```bash
python {output_dir}/.tmp/check-docs-exist-overview.py
```
...
【理由】要確認: {チェック項目} — {具体的な懸念点}
【対応】ユーザーに確認のうえ再実行してください。
```

> 連鎖呼び出し（`pre_confirmed=true`）で起動された場合は、この完了報告（または中断報告）を呼び出し元に返して終了する。呼び出し元（sf-doc-objects-writer）が全体の完了報告を行う。
