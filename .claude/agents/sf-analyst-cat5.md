...
[共通品質原則参照](../CLAUDE.md#品質原則sf-memory-全カテゴリ共通) — 以下はカテゴリ5固有の追加原則。
...
カテゴリ5 は **カテゴリ1・4の完了後に実行**される。まず `docs/.sf/_context_cache.json` が存在するか確認する:
- **存在する場合**: Read して `uc_ids` / `related_objects` / `fr_ids` / `glossary` を取得する
- **存在しない場合**: 直接 Read する: `docs/flow/usecases.md`・`docs/overview/org-profile.md`・`docs/requirements/requirements.md`

**cat3 の生成物（E-2: FG actor 精度向上）**:
- `docs/data/automation-config.md` を Read し、承認プロセスの承認者ロール・キューの対象オブジェクトを把握する。承認段階・キューから FG の actor を確定する際の補助情報として使う（承認者ロール = actor 候補）。

**cat4 の生成物（設計書参照）**:
- `docs/design/` 配下の各設計書の「担当オブジェクト」「関連UC」フィールドを参照して割り当て精度を上げる。
...
- **存在する → 差分更新モード**: 既存 YAML を Read して必須キーを確認する。`assignment_confidence` が欠落していればPhase 3 で追記する。手動修正は保持する
  - **技術識別子チェック**: [phase0.5-common.md 参照](../templates/sf-memory/phase0.5-common.md) — 適用フィールド: 各グループの `name_ja` のみ（`id` / `components` 等の識別子フィールドは適用外）
...
**`assignment_confidence` フィールド（E-6・必須）**: 各 FG の割り当て根拠強度を `high` / `medium` / `low` で出力する。UC-anchor 優先度 1〜5 から以下でマップする:

| UC-anchor 優先度 | assignment_confidence |
|---|---|
| 1（Trigger TableEnumOrId 一致） | high |
| 2（設計書の関連UC 参照） | high |
| 3（全 operated_objects 1対1マッチ） | medium |
...
変更サマリを内部に記録しておく。`docs/logs/changelog.md` への追記は sf-org-analyst Phase 7.5 で 1 セッション 1 行に集約するためここでは行わない（F-4）。
