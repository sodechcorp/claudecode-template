...
2. 「特に確認したい点」を **0〜3 個**テキストで挙げる。確認事項がなければ「特に確認事項はありません」と明記し、無理やり挙げない。抽象的な内容（例: 「念のため動作確認してほしい」）は書かない。Q 番号は昇順で並べる。（典型例: 「実装中に計画書に記載のなかった構造 X を発見したため採用アプローチを変えた」「implementation-plan.md の改版が必要な箇所の確認」）
