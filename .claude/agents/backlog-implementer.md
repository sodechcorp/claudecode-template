...
> **ダイジェスト優先（高速化）**: 次に `docs/logs/{issueID}/context-digest.md` の存在を確認する。存在する場合は Read して知識ベース・設計層コンテキストを取得し、Task tool の sf-context-loader 起動を省略する（investigator が取得済みのコンテキストを再利用）。ダイジェストが存在しない場合、またはダイジェストに実装で必要なコンポーネントの設計情報が含まれない場合のみ、以下の Task tool を起動する。
...
実装方針まとめテーブルを起点に、実装内容を把握する。**判断ポイントが0件のケース**（backlog-planner B-3 の設計により「### 実装方針まとめ」の代わりに「### 判断ポイントなし（全カテゴリ一意確定）」が出力されている場合）は、代わりに「## 関連コンポーネント一覧（変更対象ファイル）」を起点に実装内容を把握する。
...
上記マッピングを全ての変更ファイルに適用し、対象ドキュメントが存在しないケースを**先に全件洗い出してから、まとめて1回だけ確認する**（ファイルごとに都度確認しない。存在しないケースが1件もなければこの確認自体を省略する）。

- **確認文言**: 「以下の設計書が存在しません。作成しますか？（{件数}件）」として対象ドキュメントパスを一覧表示する（勝手に作成しない）
- **Yes**（全件 or 一部指定）の場合: 該当する各ドキュメントについて `sf-architect` に設計書の作成を依頼し、生成された設計書へ当該変更内容を反映する（backlog-implementer 自身が独自フォーマットで新規作成しない）
...
実装中の発見事項によって対処経路が異なる。**経路 2・3 は必ず実装を停止してユーザー確認を取る。経路 1 は実装を続行してよいが、必ず改版履歴への追記＋ Phase 末尾の【確認事項】で報告し、確認なしで方針変更したままにしない。**

### 経路 1: 計画書の微修正で対応可能（最多ケース）

- **例**: API 名のスペルミス・列挙値の追加・閾値の微調整・計画書に記載のなかった構造の採用
- **対処**: 実装を続行し、`docs/logs/{issueID}/implementation-plan.md` の末尾「改版履歴」テーブルに追記する（Phase 末尾の【確認事項】でも必ず報告する。追記だけで済ませない）
...
1. Phase 末尾の確認プロトコル（`_README.md §Phase 末尾の確認プロトコル` に従う）を単一ブロックで出力する。【Phase 4 完了サマリー】は変更ファイル数・主な変更点を中心に3〜5行で書く（別枠のサマリーを重ねて出力しない）。【確認事項】欄の Phase 4 固有の典型例（該当時のみ・0件が原則）:
...
- 削除後に確認コマンドで「削除成功」を確認してから完了報告する（残存していた場合は原因を報告する）
