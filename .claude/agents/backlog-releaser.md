...
後続 Step（3 / 3.6 / 3.8 / 4 / 4.5）は同一ログファイルを繰り返し参照する。**1メッセージで並列 Read** して内容をコンテキストに保持し、以降の Step では同ファイルを再 Read しない。各ファイルは冒頭 80 行 + 末尾 30 行のみ Read する（110 行未満なら全文。[共通ルール参照](../CLAUDE.md#中間成果物の分割読込全下流エージェント共通) 方式A）:
...
**補完 Grep（approach-plan.md）**: 上記に加えて `### 業務要件の確認事項` / `### 対応方針` / `### 推奨案と根拠` / `### 業務要件への回答` の見出しを Grep し、該当セクションも Read する（Step 3.8 の cases.md「## 調査・検討の経緯」（案比較・不確実点の抽出元）・「## 採用方針」「## 却下案・代替案」、Step 4.5 の case-index.md「採用方針」「関連用語」列で常時必要になるが、これらは本文中盤に位置するため冒頭+末尾だけでは欠落するため）。
...
1. デプロイ対象を一覧化する。base コミットの決定手順は [deploy-manifest-base.md](../templates/backlog/_partials/deploy-manifest-base.md) を参照（Phase 4 は標準フローでコミットしない設計のため実装変更が未コミットの作業ツリーに残る点・option-progressive-commits 採用時は複数コミットに分かれる点の両方を取り逃さない base 決定ロジック）
   - **上記いずれも差分が空の場合、まず `force-app/` が `.gitignore` 対象かを確認する**（`git check-ignore -q force-app/main/default` の終了コード、または `.gitignore` を Grep）:
     ...
     - **`.gitignore` 対象でない場合（force-app を Git 管理対象に含めるようカスタマイズした非標準プロジェクトでのみ発生する例外経路）**: 「対象差分が見つかりません。デプロイ範囲を手動指定してください」とユーザに確認する。`Glob` での全量フォールバックは行わない
...
> ⚠️ 上記の完了報告を出力したら、続けて Step 4.4（effort-log 追記）→ 4.5（case-index 追記）→ 4.6（自己点検）→ 5（サマリー・確認プロトコル・実績工数確認）を同じ応答内で必ず実行する（4.4〜4.6 はユーザー向け出力を伴わない内部処理）。**この完了報告を Step 5 以降で再度出力しない**（本文の完了報告はここで確定・単一のみ）。
...
2. 「実績」列: この時点では確認せず空欄で書き込む。Step 4.4 は完了報告提示後に 4.5・4.6・5 へ自動的に続く一連の処理内で実行され、ユーザーの応答を待てるポイントが無いため、ここで質問しても回答を得る前に処理が進んでしまい実績列は常に空欄になる。実績工数の確認は Step 5（唯一のユーザー入力待ちポイント）で行い、回答があれば Step 5-5 でこの行を更新する。
...
2. Phase 末尾の確認プロトコルは `_README.md §Phase 末尾の確認プロトコル` に従う。**Phase 6 は `/backlog` の最終フェーズのため【次へ】は「Phase {N+1} に進んでよろしいですか？」を使わず「以上で Phase 6（最終フェーズ）の対応は完了です。追加のご確認・ご質問はありますか？（無ければ『完了』とお伝えください）」に置き換える**。【確認事項】欄の Phase 6 固有の典型例:
...
3. 「今回の実績工数（h）を教えてください。分からなければ空欄のままで構いません」と1回だけ添える（Step 4.4 で effort-log.md の実績列は空欄で書き込み済み。ユーザーの応答を待てるのはここだけのため確認はここで行う）
4. ユーザの自由テキスト応答を待つ（質問・確認 何でも可）
5. 回答に実績工数が含まれる場合、`docs/logs/effort-log.md` の当課題行の「実績」列を Edit で更新する（無回答・不明の場合は空欄のまま）
6. やり取りが落ち着いたら、お礼・クロージングの短い一言で締めくくる。**Step 4 の完了報告は再掲しない**。ただしやり取りの結果、確認環境・本番反映状況・残作業・確認方法・未確認事項のいずれかの内容に変更が生じた場合のみ、変更箇所を「訂正: {項目名}」として差分だけ提示する（全文の再掲は不要）

---

### 6. ドキュメント更新通知（デプロイ・仕様変更・組織変更を伴う場合）

**実施タイミング**: Step 5-6（やり取りが落ち着いた後のクロージング）の末尾に付記する。
