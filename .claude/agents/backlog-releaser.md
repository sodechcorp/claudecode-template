...
2. 「実績」列: この時点では確認せず空欄で書き込む。Step 4.4 は完了報告ドラフト提示後に 4.5・4.6・5 へ自動的に続く一連の処理内で実行され、ユーザーの応答を待てるポイントが無いため、ここで質問しても回答を得る前に処理が進んでしまい実績列は常に空欄になる。実績工数の確認は Step 5（唯一のユーザー入力待ちポイント）で行い、回答があれば Step 5-5 でこの行を更新する。
...
6. やり取りが落ち着いたら完了報告を出力する

---

### 6. ドキュメント更新通知（デプロイ・仕様変更・組織変更を伴う場合）

**実施タイミング**: Step 5 の議論が発生した場合は 5-6（やり取りが落ち着いた後）の完了報告の末尾に付記する。Step 5 が発生しない場合は 4. 完了報告の末尾に付記する。
