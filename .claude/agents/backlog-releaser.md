...
**開始前に呼び出しパラメータ `auto_fix_mode` の値を確認する。**

| 状態 | モード | スキップ可能ステップ |
|---|---|---|
| `auto_fix_mode` が**未指定 または `false`**（通常の `/backlog` フロー） | **初回リリース** | なし（全ステップ実施） |
| `auto_fix_mode` が**`true`**（= `/test` F-2 自動修正ループからの再デプロイ） | **軽量再デプロイ** | お客様確認サイン（Step 3.7）・知見還流（Step 3/3.6/3.8/4.5）をスキップ |

軽量再デプロイ時は「再デプロイ（/test NG 修正後）」と冒頭に明示してからリリース手順へ進む。

> **設計変更の経緯**: 旧判定（`test-report.md` の存在有無）は、通常の `/backlog` フローでも Phase 5（backlog-tester）が Phase 6 開始前に必ず `test-report.md` を生成するため、初回リリースを含む全案件が常に「軽量再デプロイ」に誤判定される欠陥があった。`auto_fix_mode` は `/test` F-2 自動修正ループ（`test.md` F-2 Step 3 の起動パラメータ）からの起動時のみ `true` が明示的に渡される呼び出しパラメータであり、通常フローとの区別に使える。
...
3. デプロイ実行判断（2026-09-15変更: Sandbox は可逆・低リスクのため承認を求めず自動進行する）:
   - **dry-run を省略した場合**（2-3: force-app 無変更・Phase 5 PASS 済み）: 「Phase 5 で dry-run PASS 済み・force-app 無変更のためそのままデプロイを実行します」と1行通知して 4 へ進む
   - **dry-run を実行して PASS した場合**（2-4: force-app に変更あり、または PASS 記録なし）: 「dry-run PASS（{結果概要}）を確認したため、そのままデプロイを実行します」と1行通知して 4 へ進む
   - **dry-run が FAIL した場合**: デプロイは行わず中断する。dry-run 結果（変更コンポーネント一覧・エラー内容）をテキストで提示し、「dry-run FAIL のためデプロイを中断しました。Phase 4 に戻って修正しますか？」と確認する（これは業務判断ではなく壊れの報告のため、実質 Phase 4 差し戻し一択だが、修正方針をこの場でユーザーと相談したいケースを考慮しテキストで一言添える）
...
5. デプロイ後の動作確認（Phase 5 と二重チェック）:
   ...
   **ループ上限チェック**（記録前に必ず実施。`test-fail-routing.md §ループ上限` と同様の仕組み）: 既存の `docs/logs/{issueID}/release-issue.md` があれば `release-issue.R{N}.md`（N = 既存の `release-issue.R*.md` の最大回次番号 + 1。欠番があってもファイル数ではなく最大値を基準にする）へリネームして退避してから今回分を記録する。通算差し戻し回数（退避済み `release-issue.R*.md` の最大回次番号 + 今回分）が4回目以上に達している場合は「Phase 6 からの差し戻しが繰り返されています。業務担当者との打ち合わせを推奨します」とユーザーに提案し、継続・中止の判断を求める。
...
2. **（2026-09-15変更）この時点ではチャットにリマインドを出さない**: `/test` の網羅的テストがまだ実行されておらず、動作未検証のままお客様確認を促すのは早すぎるため（詳細: [customer-signoff.md §リマインドを出すタイミング](../templates/backlog/customer-signoff.md)）。代わりに `{issue_type}` に応じた要否（下記の3区分。判定基準は customer-signoff.md の表と同一）と、Step 2a-5 で生成済みの「🔎 目視確認のご案内」ブロックへの参照を `docs/logs/{issueID}/pending-signoff.md` に記録するのみに留める:
   - **バグ**、または権限・FLS・レイアウト・RecordType・共有ルール変更を含む「追加要望」「その他」: 要否 = 必須
