...
コマンドが失敗した場合（接続切れ・alias 未設定・SF CLI 不在）は、ユーザーに再認証（`sf org login web --alias <alias>`）または alias 設定を依頼してから再実行する。
...
5. デプロイ後の動作確認:
   ...
   - 問題があれば /backlog のフロー Phase 5（backlog-tester）に差し戻す。差し戻し理由・現象・ログを Backlog コメントに記録し、「Phase 5 から再開してください。/backlog を再実行して途中フェーズから再開（Phase 5）を選択してください」とユーザに案内する
...
**リリース・ロールバックシート リリース実施記録追記**（r38 = リリース実施記録の開始行。xlsx テンプレートの「リリース・ロールバック」シート構造に依存しているため、テンプレ変更時は要更新）:
```bash
python scripts/python/backlog-xlsx/update_records.py \
  --folder "{xlsx_folder}" --issue-id "{issueID}" \
...
`{issue_type}` が `バグ` かつ `{xlsx_folder}` が設定されている場合、サイン取得後に xlsx を更新する:
```bash
python scripts/python/backlog-xlsx/update_records.py \
  --folder "{xlsx_folder}" --issue-id "{issueID}" \
...
（記録する場合: 見込み {Xh} / 実績 {Yh} のように回答してください。片方のみ・不明・スキップ可。例: 見込み 3h のみ / 実績のみ不明 / スキップ）」
...
※ 見込み（非CC）は `docs/logs/{issueID}/approach-plan.md` の採用案「見込み工数（通常作業前提）」欄から取得する（Step 3 でファイルなし判定済の場合は `不明` と記録する）
...
2. 「特に確認したい点」を **0〜3 個**テキストで挙げる。確認事項がなければ「特に確認事項はありません」と明記し、無理やり挙げない。（典型例: 「本番リリースのタイミング・担当者の確認」）
