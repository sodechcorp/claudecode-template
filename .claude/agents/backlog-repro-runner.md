...
- **ファイルが存在する（Phase 1 再入ループでの再実行）**: 「検証サマリー」テーブルから既に検証済みのH番号・検証結果・採用判定・対応する「検証手順と結果」セクションの内容を控えておく（Step 8 で前回結果をそのまま転記するため）。Step 5 の検証対象は、投入された仮説リスト（H1〜Hn）のうち**このテーブルに存在しない新規H番号のみ**とする（既検証H番号は Sandbox 再検証を行わない）。
...
Step 2「再入判定」で特定した**検証対象の仮説のみ**（初回実行時は H1〜Hn 全件、再入時は新規H番号のみ）を**独立した状態**で順次検証する。仮説間で状態が持ち越されないよう、以下の原則を守る:
