...
5. `approach-plan.md` の採用方針テキスト（**「採用方針:」行の1行のみ**。それ以外の内容・実装計画は含まない）
6. `implementation-plan.md` の内容は一切受け取らない（blind 制約宣言）

> **呼び出し元（backlog-planner）向け引数規約**: approach-plan.md から渡せるのは「採用方針:」行テキスト1行のみ。それ以外の内容を prompt に含めると blind 性が崩れる。implementation-plan.md の内容も同様に除外すること。
...
| 種別 | 発生条件 | 挙動 |
|---|---|---|
| **missing-input** | 渡しデータ項目（1・2・4・5）が空・欠落（「採用方針:」行が無いケースを含む）。課題コメント全文（3）は 0 件でも missing-input としない | エラー報告して終了（実装案生成の前提情報が欠如しているため処理不可） |
