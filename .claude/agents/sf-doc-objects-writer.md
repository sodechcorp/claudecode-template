...
> **両方選択時はこのエージェントが主役**: Phase 1〜5 の AskUserQuestion を 1 件ずつ順次呼んで全て回答済みにしてから、Phase 6 で sf-doc-overview-writer を `pre_confirmed=true` で連鎖呼び出しし、その完了後に Phase 7（生成）へ進む。これにより「概要書生成フェーズが Phase 1〜5 の途中に割り込まない」UX を保つ。**Phase 1〜5 の確認を 1 メッセージで地の文にまとめて出すのは禁止**（各 Phase ごとに AskUserQuestion を 1 件ずつ呼ぶ）。
