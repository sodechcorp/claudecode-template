...
以下の内容で `{output_dir}/.tmp/check-docs-exist.py` を Write する:
```python
import pathlib
...
```
```bash
python {output_dir}/.tmp/check-docs-exist.py
```
...
以下の内容で `{output_dir}/.tmp/get-target-org-and-system-name.py` を Write する:
```python
...
```
```bash
python {output_dir}/.tmp/get-target-org-and-system-name.py
```
...
```bash
python -c "import pathlib, glob, os; files = sorted(glob.glob(r'{output_dir}/01_基本設計/オブジェクト項目定義書_v*.xlsx'), key=os.path.getmtime, reverse=True); [print(f) for f in files]; print('LATEST:', files[0] if files else '')"
```
...
```bash
python -c "import sys; sys.path.insert(0, r'{project_dir}/scripts/python/sf-doc-mcp'); from meta_store import read_meta; m = read_meta(r'{latest_obj_file}'); print(m.get('system_name', '')) if m else None"
```
...
以下の内容で `{output_dir}/.tmp/save-system-name.py` を Write する:
```python
import pathlib, sys
try:
    import yaml
...
```
```bash
python {output_dir}/.tmp/save-system-name.py
```
...
以下の内容で `{output_dir}/.tmp/list-candidate-objects.py` を Write する:
```python
import re, pathlib
...
```
```bash
python {output_dir}/.tmp/list-candidate-objects.py
```
...
以下の内容で `{output_dir}/.tmp/list-existing-objects.py` を Write する:
```python
import sys
...
```
```bash
python {output_dir}/.tmp/list-existing-objects.py
```
