...
1. `{impl_plan_path}` — 以下のセクションを抽出する:
   - 「テスト観点（軽量列挙）」— 観点の起点
   - **「変更ファイル一覧」「対応内容（実装サマリー）」「改版履歴」** — 実施した変更点の一覧（Step 3 の網羅性軸2の input）
...
> **test の対象**: 実際に処理を動かした挙動（SOQL でデータ確認、画面スクショで UI 確認）のみ。メタデータ・ファイル内容の確認は `/backlog` で完結しており、`/test` では扱わない。`メタ確認`/`ファイル確認` 種別は使用禁止。権限・レイアウト確認は UI（Login As の実画面）で行う。
...
1. `{impl_plan_path}` の「変更ファイル一覧」「対応内容」「改版履歴」を再確認する（Step 1 で読んだ内容を参照）
