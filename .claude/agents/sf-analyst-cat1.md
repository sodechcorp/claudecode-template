...
- **旧スナップショット件数の無効化**: 再実測したユーザー総数・プロファイル分布・件数を org-profile.md に書き込んだら、同一ファイル内に残る前回値を Grep して新実測値で上書きする（新旧 2 値を併存させない。[件数・数値の一貫性原則](.claude/spec/sf-memory-quality.md#件数数値の一貫性原則差分更新横断補完共通) 参照）
