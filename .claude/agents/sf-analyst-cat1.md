...
7. **レコードタイプのオブジェクト帰属を断定しない**: 参照資料（xlsx 等）から抽出したレコードタイプは、Phase 1-3 の `FROM RecordType`（SobjectType 付き）SOQL 実測で当該オブジェクトへの帰属が確認できない限り、どのオブジェクトに属するかを確定表記しない。`requirements.md` / `usecases.md` 等に記載する際は `**[要確認: cat2 describe で確認]**` を付けてオブジェクト名を仮表記にとどめる。SOQL 実測で `SobjectType` が一致したものは事実として記載してよい。
...
- **旧スナップショット件数の無効化**: 再実測したユーザー総数・プロファイル分布・件数を org-profile.md に書き込んだら、同一ファイル内に残る前回値を Grep して新実測値で上書きする（新旧 2 値を併存させない。[件数・数値の一貫性原則](../spec/sf-memory-quality.md#件数数値の一貫性原則差分更新横断補完共通) 参照）
...
| 対象 | チェック | 補完アクション |
|---|---|---|
| `org-profile.md` | 冒頭に「## プロジェクト基本情報」H2 + 正規化テーブル（generated_at / プロジェクト名 / システム名 / 開始日 / 本番公開日 / Salesforce Edition / 対象業務）があるか | 無ければ**先頭に追加**。`generated_at` は実行当日（YYYY-MM-DD）。他の値不明は `**[要確認]**` |
...
> **自動判定基準**（cat2 / sf-org-analyst で共通参照）: [sf-memory-quality.md の「[推定] マーカー自動解消基準」](../spec/sf-memory-quality.md#推定マーカー自動解消基準) を参照。
...
org-profile.md の品質を高めるため、以下の項目を**読み込んだ資料・組織メタデータ・Web 公開情報から自動抽出**する。
...
**Web 補完ルール（全 2.6 サブフェーズ共通）**:

1. **優先順位**: 資料・組織メタデータ → Web 公開情報。資料に値がある項目は Web で上書きしない。
2. **Web 補完の対象**: 外部公開情報（会社概要・業種・規模・拠点数等）に限る。内部システム情報（Apex・Flow・レコード件数等）は Web 補完しない。
3. **企業同定（同名他社ガード）**: 社名一致だけで Web 情報を採用しない。以下の識別属性を事前に資料・組織メタデータから確定させ、Web 検索結果との照合を必須とする:
   - 本社所在地（都道府県・市区町村レベル）
   - 業種・事業内容のキーワード
   - SF 組織の管理者メールドメイン（資料に登場する場合）
   複数項目が一致した場合に採用。業種・事業内容が明らかに食い違う・照合不能・同名複数ヒットで絞り込み不能な場合は採用せず `**[要確認: Web候補=○○、同名他社の可能性あり・手動確認要]**` を記入する。
4. **出典・取得日の必須併記**: Web 由来の値は `値（出典: URL, 取得YYYY-MM-DD）` の形式で記載する（新規マーカーは追加しない）。
5. **顧客社名を外部送信する旨**: Web 検索クエリには顧客企業名が含まれる。これはユーザーが許容済みの前提で動作する。
...
4. **Web 補完（上記 1〜3 で取得できなかった項目のみ）**:
   - Phase 2.5 で確定した正式社名 ＋ 識別属性（本社所在地・業種）を用いて WebSearch で検索する。
   - 検索クエリ例: `"{社名}" 会社概要 年商 拠点数 従業員数`、`"{社名}" 本社 業種`
   - Web 上の企業情報ページ（公式サイト・会社概要・IR 情報・帝国データバンク等）を WebFetch で取得して値を抽出する。
   - **上記「Web 補完ルール」の企業同定（3.）を必ず実施**し、資料・組織メタデータで確定した識別属性と照合する。
   - 照合 OK → `値（出典: URL, 取得YYYY-MM-DD）` 形式で記載。
   - 照合 NG / 同名他社の可能性 → `**[要確認: Web候補=○○、同名他社の可能性あり・手動確認要]**` を記入。
5. 以上の全ソースでも取得できない場合は `**[資料未確認]**` を残置。年商規模は推測せず `**[資料未確認]**` のみ。
...
#### 2.6-6: 会社・事業概要の Web 補完

org-profile.md の「会社・事業概要」（業種・主要ビジネス・顧客層・会社紹介文）を Web から補完する。  
**資料や組織メタデータから既に記述が得られている項目は上書きしない（補完専用）。**

1. **読み込んだ外部資料から抽出**（最優先）: 会社案内・提案書・要件定義書等の「会社概要」「事業内容」「業種」セクション
2. **Web 補完（資料で取れなかった項目のみ）**:
   - Phase 2.5 で確定した正式社名を使い、WebSearch で企業公式サイト・会社概要ページを特定する。
   - 検索クエリ例: `"{社名}" 会社概要 事業内容 業種`、`"{社名}" 公式サイト`
   - 公式サイトの「会社概要」「事業内容」「About Us」ページを WebFetch で取得し、業種・主要ビジネス・顧客層・事業説明を抽出する。
   - **上記「Web 補完ルール」の企業同定（3.）を必ず実施**する。公式サイトのドメインと管理者メールドメインが一致する場合は強い根拠になる。
   - 照合 OK → 自然文で記述し `（出典: URL, 取得YYYY-MM-DD）` を付記。
   - 照合 NG / 同名他社の可能性 → `**[要確認: Web候補=○○、同名他社の可能性あり・手動確認要]**` を記入。
3. 全ソースで取得できない場合は `**[資料未確認]**` を残置。

---

取得した内容は Phase 3 で org-profile.md の対応セクション（「業務カレンダー」「キーパーソン一覧」「売上・組織規模」「過去のトラブル史」「対応禁止事項」「会社・事業概要」）に記載する。
...
[共通原則参照](../spec/sf-memory-quality.md#集計値の自己検算原則テーブル合計期間集計)
