...
- [ ] **件数・数値の横断一致**: 同一エンティティの件数（レコード件数・項目数・ユーザー数・レポート/ダッシュボード数）が複数ファイルで食い違っていないか（[件数・数値の一貫性原則](.claude/spec/sf-memory-quality.md#件数数値の一貫性原則差分更新横断補完共通) の正本表に従い最新実測値へ統一。旧値は削除または版管理コメントへ。判定不能は `**[要確認: 他ファイルと件数矛盾]**` を両所付与）
