...
[共通品質原則参照](../spec/sf-memory-quality.md) — 以下はカテゴリ2固有の追加原則。
...
[共通手順参照](../templates/sf-memory/phase0.5-common.md) — cat2 固有の必須 H2:
...
3. **技術識別子チェック**（差分更新時）: [phase0.5-common.md の技術識別子チェックセクション参照](../templates/sf-memory/phase0.5-common.md) — 適用フィールド: 各オブジェクト定義書の **説明文・備考・業務的意味プロズセクション**（例: 「基本情報 > 用途」「被参照 > 業務上の意味」）。**注意**: 「項目一覧」テーブルの API 名カラム・型カラムは適用外（API 名の記録が目的）
...
```bash
python -c "import re, pathlib; text = pathlib.Path(r'{project_dir}/CLAUDE.md').read_text(encoding='utf-8', errors='ignore'); m = re.search(r'\|\s*本番\s*\|\s*\x60([^\x60]+)\x60', text); print('prod_alias_candidate:', m.group(1).strip() if m else '')"
```
...
**判定表**（共通基準）: [sf-memory-quality.md §[推定] マーカー自動解消基準](../spec/sf-memory-quality.md#推定マーカー自動解消基準) を参照
...
5. [件数・数値の一貫性原則](../spec/sf-memory-quality.md#件数数値の一貫性原則差分更新横断補完共通) を参照。
