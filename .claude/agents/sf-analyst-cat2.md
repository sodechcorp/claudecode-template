...
1. 正本は各オブジェクト定義書の `基本情報テーブル`（無ければ `data/data-statistics.md`）。[件数・数値の一貫性原則](.claude/spec/sf-memory-quality.md#件数数値の一貫性原則差分更新横断補完共通) の正本階層に従う。
...
- **件数の最新化**: Phase 3 で再実測したレコード件数（基本情報テーブル）を正本とし、同一ファイルの用途欄・所見欄・本文に残る旧件数を Grep して正本値で上書きする。数値のみ差し替え、手動の判断コメントは保持する（[件数・数値の一貫性原則](.claude/spec/sf-memory-quality.md#件数数値の一貫性原則差分更新横断補完共通) 参照）
