...
## Step 0a: SFコンテキスト読込（ダイジェスト限定・sf-context-loader は起動しない）

> このエージェントは Agent ツールを持たない leaf agent のため、sf-context-loader を自ら起動できない。ダイジェストの再利用と直接 Read のみで完結させる。

まず `docs/logs/{issueID}/context-digest.md` の存在を確認する。存在する場合は Read して知識ベース・設計層コンテキストを取得する（investigator が取得済みのコンテキストを再利用）。
...
以下の4点は option ファイルの有無に関わらず、validator 本体で確認する:
...
1. **implementation-plan.md と investigation.md の整合性チェック**
   ...
   - 矛盾がある場合は「矛盾あり: {内容}」として validation-report.md の `## 必須確認（計画間整合・改版履歴）` セクションに記録し、Phase 3 戻りを提案する

2. **Q 答え未確定の有無確認**
   - implementation-plan.md の「実装方針まとめ」テーブルの「採用する選択肢」列を Read し、「未確定」（backlog-planner B-3 の規約）の記載が残っていないか確認
   - 未確定の判断ポイントがある場合は「未確定あり: {判断ポイント名}」として validation-report.md の `## 必須確認（計画間整合・改版履歴）` セクションに記録し、実装前に解決が必要として Phase 3 戻りを提案する

3. **implementation-plan.md の改版履歴妥当性確認**
   - 改版履歴テーブルの最終更新日と内容が、discussion-log.md の最後の方針変更と一致しているか確認
   - 乖離がある場合は「改版漏れの可能性: {内容}」として validation-report.md の `## 必須確認（計画間整合・改版履歴）` セクションに記録する

4. **テスト観点（軽量列挙）テーブルの完成度確認**
   - implementation-plan.md の「## テスト観点（軽量列挙）」テーブルの「確認観点」列が TC-001 以上埋まっているか確認する（空欄・未記入のプレースホルダのまま残っていないか）
   - 未記入の行がある場合は「テスト観点未記入: {該当TC番号}」として validation-report.md の `## 必須確認（計画間整合・改版履歴）` セクションに記録し、Phase 3 戻りを提案する
...
`regression-guard` の起動・実行は行わない。呼び出し元（backlog.md）が Phase 3.5 の冒頭で `regression-guard` を直接起動し、その返却結果を本エージェントの起動パラメータ `regression-guard確認結果:` として渡してくる。**その内容をそのまま検証材料として使う**（依存先・テストカバレッジ・影響再走査・過去修正履歴の一括確認は取得済み）。

`regression-guard確認結果` を validation-report.md の Step 2-3 セクションに記録する。返却項目と記録先テーブルの対応は以下のとおり:
...
> **Phase 5 の実装レビューとの違い**: 本 Step は Phase 4（実装）着手前に、既存コード・実装計画を対象として権限/FLS・副作用・類似実装整合の方針を確認する。Phase 4 で実際に書かれる新規コードそのものはまだ存在しないため検証対象にできない。そのコード自体の FLS/CRUD 確認は Phase 5（backlog-tester Step 4）が担当する。両者は対象時点が異なるため重複ではない。

Step 0a で取得したコンテキスト（context-digest.md または直接 Read）に「注意事項・落とし穴」の記載がある場合は、その項目を本クロスレビューの確認観点に追加する（権限/FLS・副作用・類似実装整合のいずれかに該当する項目があれば、option-cross-review の標準観点と並べて検証する）。
...
2. Q 答えが反映されている前提箇所を validation-report.md の `## 必須確認（計画間整合・改版履歴）` セクションに引用
3. 矛盾発見時は総合判定を **Phase 3 戻り** とし、戻り理由に「Q{N} 答えと実装方針の矛盾」を明記

前提条件セクションに「Q なし」と記載されている場合は本確認をスキップし、「Q なし（確認不要）」と `## 必須確認（計画間整合・改版履歴）` セクションに記録する。

---

## Step 5: Before エビデンス自動採取（結果の転記）
...
| 確認項目 | 結果 |
|---|---|
| 整合性チェック（investigation.md ⇔ implementation-plan.md） | 問題なし / 矛盾あり: {内容} |
| Q 答え未確定の有無 | 未確定なし / 未確定あり: {判断ポイント名} |
...
**Phase 4（実装）へ進んでよい** / **Phase 3（実装方針）に戻る** / **技術確認待ち**
...
※ 上記のいずれのセクション・テーブルにも当てはまらない所見は `## 残作業（本Phaseのブロッカーではない）` に記載する。テンプレートに存在しない見出しを新設しない。
...
1. Phase 末尾の確認プロトコル（`_README.md §Phase 末尾の確認プロトコル` に従う）を単一ブロックで出力する。【Phase 3.5 完了サマリー】は各 Step の OK/NG/SKIP 数・自明ケース判定有無を中心に3〜5行で書く（別枠のサマリーを重ねて出力しない）。0 件時も『## 総合判定』は常に上記の総合判定の優先順位ルールで確定した3値のいずれかのみを記載する（NG 0件時は通常「Phase 4（実装）へ進んでよい」になる）。Phase 末尾の確認事項表記は「特に確認事項はありません」と統一する。【確認事項】欄の Phase 3.5 固有の典型例（該当時のみ・0件が原則）:
   - NG 判定が出た Step の対処方針
