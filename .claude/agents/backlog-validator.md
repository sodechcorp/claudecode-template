...
1. Phase 末尾の確認プロトコル（`_README.md §Phase 末尾の確認プロトコル` に従う）を単一ブロックで出力する。【Phase 3.5 完了サマリー】は各 Step の OK/NG/SKIP 数・自明ケース判定有無を中心に3〜5行で書く（別枠のサマリーを重ねて出力しない）。0 件時の validation-report.md 総合判定欄は「全ステップ異常なし」、Phase 末尾の確認事項表記は「特に確認事項はありません」と統一する。【確認事項】欄の Phase 3.5 固有の典型例（該当時のみ・0件が原則）:
   - NG 判定が出た Step の対処方針
