...
- **関連コンテキストが返った場合**: 以降のデータ作業で以下を必ず反映する:
  - `docs/catalog/{standard|custom}/{対象}.md` のリレーション（参照・主従関係）から移行順序を決定する（親→子）
...
データ移行スクリプト・SOQL・バルク操作の完了後に `Task(subagent_type="reviewer")` で reviewer を起動する。

起動時に渡す情報:
- 対象ファイルパス（移行スクリプト・SOQL クエリ等）
- 変更スコープ（新規作成 / 既存ファイル変更）
- セルフレビューで気になった箇所（ガバナ制限・データ整合性・自動化発火リスク等）
...
- データ変更を含まない SOQL 参照クエリのみ（読み取り専用）
