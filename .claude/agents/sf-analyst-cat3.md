...
> **セキュリティ原則（絶対厳守）**: 「データの中身」ではなく「データの構造・定義・統計」を記録する。取引先・連絡先・リード・商談の**実レコード**の個人情報・具体的な金額・担当者名（顧客側の実担当者名）は**絶対に記録しない**。マスタ系（設定値・コード値・商品情報等）のみ対象。なお FolderName・承認者・個人所有フォルダ等の**メタデータ名に含まれる個人名は、社内利用前提のため記録してよい**（伏字不要。実レコードの担当者名・メール等の記録禁止とは区別する）。
...
[共通品質原則参照](../CLAUDE.md#品質原則sf-memory-全カテゴリ共通) — 以下はカテゴリ3固有の追加原則。
...
[共通手順参照](../templates/sf-memory/phase0.5-common.md) — cat3 固有の必須 H2:
...
3. **技術識別子チェック**（差分更新時）: [phase0.5-common.md の技術識別子チェックセクション参照](../templates/sf-memory/phase0.5-common.md) — 適用フィールド: `master-data.md` の説明列・`automation-config.md` のフロー・承認プロセス説明文。**注意**: DeveloperName カラム・件数カラムは適用外
...
### Phase 6.5: 集計値の自己検算（必須）

[共通原則参照](../spec/sf-memory-quality.md#集計値の自己検算原則テーブル合計期間集計)

以下を automation-config.md / data-statistics.md / reports-dashboards.md に対して実施する:

1. **automation-config.md の合計行確認**: 各設定種別（WorkflowRule / ProcessBuilder / Flow 等）の Active 件数合計行を内訳列の和と照合する。不一致なら内訳の和で合計行を上書き。
2. **data-statistics.md の期間集計確認**: 「+N件（M日）」と「+X件/日」を両方記述している箇所を Grep し、`X × M ≈ N` を確認する。50% 超の乖離は信頼できる方のみ残し他方を `**[要確認: X/日 × M日 ≠ N件]**` にする。
3. **reports-dashboards.md の総数 vs 内訳の和確認**: レポート・ダッシュボードの総件数とフォルダ別内訳の件数和を照合する。不一致の場合、FolderName が設定されていない（個人所有フォルダ等）レコードが内訳に現れないケースがあるため、内訳の和で総数を上書きしない。総数に `**[要確認: フォルダ別内訳の和と不一致（フォルダ未設定レコードの可能性）]**` を付与する。
...
- **集計値の最新化・横断整合**: data-statistics.md・reports-dashboards.md 等の集計値は再実測値で更新し、更新前の既存値と比較する。他カテゴリの正本（[件数・数値の一貫性原則](../spec/sf-memory-quality.md#件数数値の一貫性原則差分更新横断補完共通) の表）と食い違う場合は正本に揃える
- **再計測スキップ時のマーカー付与**: 今回のアップデートで再実測しなかった集計セクション（前回値を据え置いた箇所）は「変化なし」と記述しない（「変化なし」は今回再計測して同値だった場合のみ使用可）。据え置き値には `**[要確認: 今回再計測なし・{前回測定日} 時点値]**` を付与し、再計測して同値だった「変化なし」と明確に区別する。なお `**[組織未調査]**` は一度も取得していない項目に予約するマーカーのため、過去に取得済みの据え置き値には使わない（[spec マーカー規約](../spec/sf-memory-quality.md#マーカー規約) に準拠）
