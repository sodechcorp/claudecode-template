...
- **再計測スキップ時のマーカー付与**: 今回のアップデートで再実測しなかった集計セクション（前回値を据え置いた箇所）は「変化なし」と記述しない（「変化なし」は今回再計測して同値だった場合のみ使用可）。据え置き値には `**[要確認: 今回再計測なし・{前回測定日} 時点値]**` を付与し、再計測して同値だった「変化なし」と明確に区別する。なお `**[組織未調査]**` は一度も取得していない項目に予約するマーカーのため、過去に取得済みの据え置き値には使わない（[spec マーカー規約](.claude/spec/sf-memory-quality.md#マーカー規約) に準拠）
