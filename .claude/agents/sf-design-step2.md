...
```bash
python -c "import tempfile, pathlib; tmp = pathlib.Path(tempfile.mkdtemp(prefix='sf-design-step2-')); print('tmp_dir:' + str(tmp).replace(chr(92), '/'))"
```
...
以下の内容で `{tmp_dir}/load_feature_list.py` を Write する:
```python
...
```
```bash
python {tmp_dir}/load_feature_list.py
```
...
以下の内容で `{tmp_dir}/resolve_target_group_ids.py` を Write する:
```python
...
```
```bash
python {tmp_dir}/resolve_target_group_ids.py
```
...
```bash
python -c "import json, pathlib; fl = json.loads(pathlib.Path(r'{tmp_dir}/feature_list.json').read_text(encoding='utf-8')); apex_types = {'Apex', 'Batch', 'Integration', 'Trigger', 'Flow'}; screen_types = {'LWC', '画面フロー', 'Visualforce', 'Aura'}; apex_list = [f for f in fl if f.get('type') in apex_types]; screen_list = [f for f in fl if f.get('type') in screen_types]; print(f'Apex系（sf-design-writer対象）: {len(apex_list)}件'); print(f'画面系（sf-screen-writer対象）: {len(screen_list)}件')"
```

> **件数チェック＆バッチ分割**:
>
> 以下の内容で `{tmp_dir}/split_batches.py` を Write する:
> ```python
> import json, math
...
> ```
> ```bash
> python {tmp_dir}/split_batches.py
> ```
>
