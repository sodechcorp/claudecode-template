...
DEFAULT_BRANCH="main"
# UPGRADE_TMP_DIR が環境変数で渡された場合は既存クローンを再利用（自己更新後の再実行時）
...
# --- upgrade.sh 自己更新チェック（旧バージョン検出時は新バージョンで即時再実行） ---
_SELF="${BASH_SOURCE[0]:-$0}"
if [ -f "$TMP_DIR/scripts/upgrade.sh" ] && ! diff -q "$_SELF" "$TMP_DIR/scripts/upgrade.sh" >/dev/null 2>&1; then
    info "upgrade.sh を新バージョンに更新して再実行します..."
    cp "$TMP_DIR/scripts/upgrade.sh" "$_SELF"
    trap - EXIT  # TMP_DIR を保持したまま再実行するためトラップ解除
    UPGRADE_TMP_DIR="$TMP_DIR" exec bash "$_SELF" "$@"
fi
